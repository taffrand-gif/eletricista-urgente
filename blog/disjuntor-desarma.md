---
title: "Disjuntor Desarma Sempre: 7 Causas e Como Resolver (2026)"
slug: "disjuntor-desarma"
category: "Problemas Elétricos"
date: "2026-03-04"
author: ""
excerpt: "Disjuntor desarma constantemente? Descubra as 7 causas principais (desde sobrecarga até curto-circuito) e como resolver. Guia completo com diagnóstico e soluções seguras."
keywords: ["disjuntor desarma", "disjuntor salta", "luz vai abaixo", "curto circuito", "eletricista Bragança", "quadro elétrico"]
priority: "CRITICAL"
---

# Disjuntor Desarma Sempre: 7 Causas e Como Resolver (2026)

**O disjuntor desarma constantemente e fica sem luz?** Este é um problema crítico que indica sobrecarga, curto-circuito ou falha elétrica. Neste guia completo, vou mostrar-lhe **as 7 causas principais** de disjuntores que desarmam e **como resolver cada uma com segurança**.

> ⚠️ **PERIGO: Risco de incêndio!** Se disjuntor desarma repetidamente, NÃO force. Ligue já: **[932 321 892](tel:+351932321892)** — Eletricista certificado em Bragança Atendimento prioritário, Atendimento 24h/7d.

## 🔍 O Que É um Disjuntor e Como Funciona?

### Tipos de Disjuntores

**1. Disjuntor Magnetotérmico (Proteção contra sobrecarga e curto-circuito)**

**Como funciona:**
- **Proteção térmica:** Desliga se corrente > capacidade (ex: 16A) por tempo prolongado
- **Proteção magnética:** Desliga instantaneamente se curto-circuito (corrente muito alta)

**Capacidades comuns:**
- 10A: Iluminação
- 16A: Tomadas normais
- 20A: Tomadas cozinha
- 25A: Forno, máquina lavar roupa
- 32A: Placa vitrocerâmica, ar condicionado

**2. Disjuntor Diferencial (Proteção contra fugas de corrente)**

**Como funciona:**
- Deteta diferença entre corrente que entra (fase) e sai (neutro)
- Se diferença > 30mA = fuga para terra (risco de eletrocussão)
- Desliga instantaneamente

**Sensibilidades:**
- **30mA:** Proteção de pessoas (obrigatório em casas)
- **300mA:** Proteção de equipamentos (indústria)

**3. Disjuntor Geral (Proteção total)**

Desliga toda a instalação. Geralmente 40-63A.

## 🔧 7 Causas Principais: Disjuntor Desarma

### Causa 1: Sobrecarga no Circuito (40% dos casos)

**Como identificar:**
- Disjuntor desarma quando liga vários aparelhos ao mesmo tempo
- Desarma após alguns minutos (não instantaneamente)
- Disjuntor fica quente ao toque
- Acontece mais no inverno (aquecedores)

**Por que acontece:**
Está a pedir mais corrente do que o circuito suporta. Exemplo: circuito 16A (3.680W) com aquecedor 2.000W + chaleira 2.000W = 4.000W = sobrecarga!

**Cálculo simples:**
- **Potência máxima = Amperagem × 230V**
- Circuito 16A = 16 × 230 = 3.680W
- Circuito 20A = 20 × 230 = 4.600W
- Circuito 25A = 25 × 230 = 5.750W

**Solução A: Distribuir aparelhos (Grátis)**

**Não ligue ao mesmo tempo:**
- Aquecedor + chaleira
- Forno + micro-ondas
- Máquina lavar + secador
- Ferro + aquecedor

**Distribua por circuitos diferentes** (tomadas de divisões diferentes geralmente estão em circuitos diferentes).

**Solução B: Aumentar capacidade do circuito (€80-200)**

**Requer eletricista certificado:**
1. Substituir disjuntor por um de maior capacidade (16A → 20A ou 25A)
2. **MAS:** Verificar que o cabo suporta (cabo 2,5mm² suporta 20A, cabo 4mm² suporta 25A)
3. Se cabo for fino (1,5mm²), precisa substituir cabo também (€150-500)

**Solução C: Criar circuito adicional (€150-400)**

Para aparelhos potentes (ar condicionado, forno), criar circuito dedicado.

### Causa 2: Curto-Circuito (25% dos casos)

**Como identificar:**
- Disjuntor desarma INSTANTANEAMENTE ao ligar
- Faísca ou estalo ao ligar aparelho
- Cheiro a queimado
- Tomada ou ficha derretida/queimada

**Por que acontece:**
Fase e neutro tocam-se diretamente (sem resistência), criando corrente enorme (centenas de amperes). Causas: cabo danificado, aparelho avariado, roedores, água em tomada.

**⚠️ MUITO PERIGOSO: Risco de incêndio!**

**Solução: Identificar e eliminar o curto (€60-300)**

**Diagnóstico:**
1. Desligue TODOS os aparelhos do circuito
2. Tente ligar o disjuntor
3. **Se ligar:** Problema é num aparelho
4. **Se não ligar:** Problema na instalação

**Se problema é num aparelho:**
1. Ligue aparelhos um a um
2. Quando desarmar = aparelho avariado
3. Não use esse aparelho (risco de incêndio)
4. Repare ou substitua

**Se problema é na instalação:**
- **NÃO tente resolver sozinho!**
- Chame eletricista urgente: **[932 321 892](tel:+351932321892)**
- Pode ser: cabo partido, tomada danificada, ligação solta, água infiltrada
- Custo arranjo: €60-300

### Causa 3: Fuga de Corrente (Diferencial Desarma) (20% dos casos)

**Como identificar:**
- Disjuntor DIFERENCIAL desarma (não o magnetotérmico)
- Desarma quando liga aparelho específico
- Ou desarma aleatoriamente (chuva, humidade)
- Botão TEST do diferencial funciona

**Por que acontece:**
Corrente está a "fugir" para terra através de: isolamento danificado, aparelho com fuga, humidade, cabo roído.

**Teste do botão TEST:**
1. Pressione botão TEST do diferencial
2. Deve desarmar imediatamente
3. Se não desarmar = diferencial avariado (substituir)

**Solução: Identificar fonte da fuga (€60-250)**

**Diagnóstico:**
1. Desligue todos os aparelhos
2. Ligue o diferencial
3. **Se ligar:** Problema é num aparelho
4. **Se não ligar:** Problema na instalação

**Se problema é num aparelho:**
1. Ligue aparelhos um a um
2. Quando desarmar = aparelho com fuga
3. Não use (risco de choque)
4. Repare ou substitua

**Aparelhos comuns com fugas:**
- Máquina de lavar (resistência com fuga)
- Termoacumulador (resistência danificada)
- Frigorífico velho (isolamento degradado)
- Ferro de engomar (cabo danificado)

**Se problema é na instalação:**
- Humidade em caixas de derivação
- Cabo danificado em parede
- Tomada com infiltração de água
- Requer eletricista: €80-250

### Causa 4: Disjuntor Defeituoso ou Velho (8% dos casos)

**Como identificar:**
- Disjuntor desarma sem razão aparente
- Desarma com carga muito abaixo da capacidade
- Disjuntor tem > 15-20 anos
- Mecanismo está gasto ou solto

**Por que acontece:**
Disjuntores têm vida útil de 15-25 anos. Componentes internos desgastam-se e ficam mais sensíveis.

**Teste:**
1. Desligue todos os aparelhos
2. Ligue o disjuntor
3. Ligue apenas uma lâmpada (100W)
4. Se desarmar = disjuntor defeituoso

**Solução: Substituir disjuntor (€40-100)**

**Custos:**
- Disjuntor magnetotérmico 16A: €8-20
- Disjuntor diferencial 30mA 40A: €25-60
- Mão de obra: €40-80
- Total: €50-140

**Requer eletricista certificado** (trabalho no quadro elétrico).

### Causa 5: Ligação Solta ou Oxidada (4% dos casos)

**Como identificar:**
- Disjuntor desarma intermitentemente
- Cheiro a queimado no quadro
- Disjuntor ou cabos muito quentes
- Sinais de queimadura no quadro

**Por que acontece:**
Parafuso do disjuntor solto ou terminal oxidado cria resistência. Aquece, expande, perde contacto, desarma.

**⚠️ PERIGO: Risco de incêndio no quadro!**

**Solução: Reapertar ou limpar ligações (€60-150)**

**NÃO faça sozinho!** Trabalho no quadro elétrico é perigoso.

**Eletricista vai:**
1. Desligar disjuntor geral
2. Verificar todas as ligações
3. Reapertar parafusos
4. Limpar oxidação
5. Verificar temperatura após religar

### Causa 6: Problema no Neutro (2% dos casos - GRAVE)

**Como identificar:**
- Disjuntor diferencial desarma constantemente
- Lâmpadas piscam ou queimam frequentemente
- Alguns aparelhos não funcionam bem
- Tensão varia muito (180V-250V)

**Por que acontece:**
Neutro solto, partido ou com mau contacto. Corrente não retorna corretamente, diferencial deteta desequilíbrio.

**⚠️ MUITO PERIGOSO: Risco de eletrocussão e incêndio!**

**Solução: Arranjar neutro (€80-300)**

**EMERGÊNCIA - Chame eletricista imediatamente:**
- Não use eletricidade
- Desligue disjuntor geral
- Ligue: **[932 321 892](tel:+351932321892)**

**Pode ser:**
- Neutro solto no quadro
- Neutro partido em caixa de derivação
- Problema no contador (contactar EDP)

### Causa 7: Trovoada ou Sobretensão (1% dos casos)

**Como identificar:**
- Disjuntor desarma durante trovoada
- Ou após corte de luz da EDP
- Vários aparelhos avariados ao mesmo tempo
- Cheiro a queimado em vários pontos

**Por que acontece:**
Raio próximo ou manobra na rede cria sobretensão (> 300V). Danifica aparelhos e pode desarmar disjuntores.

**Solução imediata: Verificar danos (€0-1.000+)**

1. Não ligue o disjuntor imediatamente
2. Desligue todos os aparelhos das tomadas
3. Ligue o disjuntor
4. Teste aparelhos um a um
5. Aparelhos avariados: não use (risco de incêndio)

**Prevenção: Protetor de sobretensão (€80-200)**

Instalar protetor de sobretensão no quadro (obrigatório desde 2006 em instalações novas).

## 💰 Custos de Arranjo

### Soluções DIY (Faça Você Mesmo)

| Solução | Custo | Dificuldade | Risco |
|---------|-------|-------------|-------|
| Distribuir aparelhos | Grátis | Fácil | Baixo |
| Identificar aparelho avariado | Grátis | Fácil | Baixo |
| Substituir aparelho | €30-500 | Fácil | Baixo |

**NÃO faça sozinho:**
- Trabalhos no quadro elétrico
- Arranjo de curto-circuitos
- Substituição de cabos
- Qualquer trabalho com corrente ligada

### Soluções Profissionais

| Serviço | Preço em Trás-os-Montes |
|---------|-------------------------|
| Diagnóstico completo | €40-80 |
| Substituir disjuntor | €50-140 |
| Arranjar curto-circuito simples | €60-150 |
| Arranjar curto-circuito em parede | €150-400 |
| Aumentar capacidade circuito | €80-200 |
| Criar circuito novo | €150-400 |
| Arranjar fuga de corrente | €80-250 |
| Reapertar ligações quadro | €60-120 |
| Arranjar problema no neutro | €80-300 |
| Instalar protetor sobretensão | €120-250 |
| Urgência 24h | +€50-80 |

> 💡 **Disjuntor desarma constantemente?** Eletricista certificado: **[932 321 892](tel:+351932321892)** — Diagnóstico Atendimento prioritário, arranjo urgente.

## 🔍 Como Diagnosticar o Problema

### Passo 1: Identificar Qual Disjuntor Desarma

**Disjuntor geral:**
- Toda a casa fica sem luz
- Problema grave (sobrecarga total, curto grave)

**Disjuntor diferencial:**
- Vários circuitos ficam sem luz
- Fuga de corrente em algum circuito

**Disjuntor magnetotérmico individual:**
- Só um circuito fica sem luz (ex: tomadas sala)
- Problema nesse circuito específico

### Passo 2: Quando Desarma?

**Ao ligar aparelho específico:**
- Problema é esse aparelho (avariado ou potência excessiva)

**Após alguns minutos:**
- Sobrecarga (soma de aparelhos > capacidade)

**Instantaneamente ao ligar disjuntor:**
- Curto-circuito na instalação

**Aleatoriamente:**
- Disjuntor defeituoso, ligação solta, ou fuga intermitente

### Passo 3: Teste de Isolamento

**Objetivo:** Saber se problema é aparelho ou instalação.

1. Desligue TODOS os aparelhos
2. Tente ligar o disjuntor
3. **Se ligar:** Problema é num aparelho (ligue um a um para identificar)
4. **Se não ligar:** Problema na instalação (chame eletricista)

### Passo 4: Verificar Sinais de Perigo

**Chame eletricista IMEDIATAMENTE se:**
🚨 Cheiro a queimado  
🚨 Fumo no quadro ou tomadas  
🚨 Faíscas  
🚨 Disjuntor muito quente  
🚨 Cabos derretidos  
🚨 Sinais de queimadura  

**NÃO tente ligar o disjuntor!**

## 🛡️ Prevenção

### Evitar Sobrecargas

**Regras simples:**
1. Não ligue vários aparelhos potentes ao mesmo tempo
2. Use extensões com proteção (não correntes simples)
3. Distribua aparelhos por circuitos diferentes
4. Aparelhos > 2.000W: circuito dedicado

**Aparelhos que consomem muito:**
- Aquecedor: 1.500-2.500W
- Forno: 2.000-3.500W
- Placa vitrocerâmica: 3.000-7.000W
- Máquina lavar roupa: 2.000-2.500W
- Secador roupa: 2.000-3.000W
- Ar condicionado: 1.000-3.000W
- Chaleira: 2.000-3.000W
- Ferro engomar: 1.500-2.500W

### Manutenção Preventiva

**Anual:**
- Verificar quadro elétrico (sinais de aquecimento, oxidação)
- Testar botão TEST dos diferenciais
- Verificar que disjuntores ligam/desligam suavemente
- Apertar parafusos das ligações (se soltos)

**A cada 5 anos:**
- Inspeção completa por eletricista
- Termografia (detetar pontos quentes)
- Teste de isolamento

**A cada 15-20 anos:**
- Substituir disjuntores (fim de vida útil)
- Atualizar quadro se muito antigo

### Proteção Adicional

**Protetor de sobretensão (€80-200):**
- Obrigatório em instalações novas (desde 2006)
- Protege contra raios e sobretensões da rede
- Instalar no quadro elétrico

**Disjuntor diferencial 30mA:**
- Obrigatório em todas as casas
- Protege contra eletrocussão
- Testar mensalmente (botão TEST)

## ❓ Perguntas Frequentes

**1. Posso forçar o disjuntor a ficar ligado?**
**NÃO!** Nunca bloqueie ou force um disjuntor. Ele desarma para proteger contra incêndio ou eletrocussão. Forçar = risco de morte ou incêndio. Resolva a causa do problema.

**2. Disjuntor desarma à noite, porquê?**
Geralmente: termoacumulador a aquecer + outros aparelhos = sobrecarga. Ou frigorífico velho com fuga (pior à noite quando compressor liga). Ou humidade (orvalho) em instalação exterior.

**3. Posso substituir disjuntor por um de maior capacidade?**
Só se o cabo suportar. Cabo 1,5mm² = máximo 16A. Cabo 2,5mm² = máximo 20A. Cabo 4mm² = máximo 25A. Colocar disjuntor maior que o cabo suporta = risco de incêndio (cabo aquece mas disjuntor não desarma).

**4. Diferencial desarma quando chove, é normal?**
Não. Indica infiltração de água em caixa de derivação, tomada exterior, ou cabo danificado. Água cria fuga para terra. Precisa localizar e arranjar infiltração. Não ignore (risco de choque).

**5. Quanto tempo dura um disjuntor?**
15-25 anos com uso normal. Fatores que reduzem vida útil: sobrecargas frequentes, curto-circuitos, ambiente húmido, má qualidade. Substitua se > 20 anos ou funciona mal.

**6. Disjuntor aquece, é normal?**
Aquecimento leve é normal sob carga. Aquecimento excessivo (não consegue tocar) indica: sobrecarga, ligação solta, ou disjuntor subdimensionado. Desligue e chame eletricista (risco de incêndio).

**7. Posso ter vários diferenciais?**
Sim, recomendado! Diferencial por zona (ex: um para quartos, outro para cozinha). Se um desarmar, resto da casa continua com luz. Facilita diagnóstico. Custo: +€30-60 por diferencial.

**8. O que fazer se disjuntor desarma e não liga mais?**
Pode estar bloqueado por segurança. Tente: (1) desligue todos os aparelhos, (2) aguarde 5 minutos, (3) tente ligar. Se não ligar: curto-circuito grave ou disjuntor avariado. Chame eletricista urgente.

## 🎯 Conclusão: Disjuntor Que Desarma Precisa Atenção

Disjuntor que desarma constantemente é **sinal de problema elétrico** que precisa ser resolvido:

**Causas mais comuns:**
1. **Sobrecarga** (40%) → Distribuir aparelhos ou aumentar capacidade
2. **Curto-circuito** (25%) → Identificar e arranjar (URGENTE)
3. **Fuga de corrente** (20%) → Localizar aparelho ou ponto com fuga
4. **Disjuntor defeituoso** (8%) → Substituir

**Sinais de EMERGÊNCIA:**
🚨 Cheiro a queimado  
🚨 Fumo ou faíscas  
🚨 Disjuntor muito quente  
🚨 Cabos derretidos  

**Quando chamar eletricista:**
- Não consegue identificar a causa
- Problema na instalação (não em aparelho)
- Sinais de perigo (queimado, fumo, calor)
- Precisa trabalhar no quadro elétrico

**NUNCA:**
❌ Force o disjuntor a ficar ligado  
❌ Substitua por disjuntor de maior capacidade sem verificar cabo  
❌ Ignore o problema (risco de incêndio)  
❌ Trabalhe no quadro com corrente ligada  

### 🚨 Disjuntor Desarma Constantemente? Ajuda Urgente

** — Eletricista Certificado em Bragança**

📞 **Ligue agora: [932 321 892](tel:+351932321892)**  
💬 **WhatsApp: [Clique aqui](https://wa.me/351932321892?text=Olá%2C%20o%20meu%20disjuntor%20desarma%20constantemente.%20Preciso%20de%20ajuda%20urgente!)**

✅ Atendimento Atendimento 24h/7d (urgências elétricas)  
✅ Diagnóstico rápido Atendimento prioritário  
✅ Arranjo imediata  
✅ Equipamento de deteção profissional  
✅ Garantia 1 ano no serviço  
✅ Fichas eletrotécnicas assinadas por técnico certificado DGEG se necessário  
✅ Cobrimos Bragança, Mirandela, Macedo de Cavaleiros e toda a região

**Não arrisque incêndio ou eletrocussão!** Disjuntor que desarma é aviso de problema grave. Contacte-nos já para intervenção urgente.

---

**Artigos relacionados:**
- [Lâmpada LED Pisca: 7 Causas e Soluções](/blog/lampada-led-pisca)
- [Protetor de Sobretensão: O Que É](/blog/protecao-sobretensao)
- [Como Instalar Tomada no Exterior](/blog/instalar-tomada-exterior)
- [Como Instalar Ventilador de Teto](/blog/instalar-ventilador-teto)
- [Eletricista em Bragança 24h](/eletricista-braganca)
