---
title: "Eletrocussão Prevenir: Segurança Elétrica Casa e Trabalho"
slug: "eletrocussao-prevenir"
category: "seguranca"
date: "2026-03-04"
author: ""
excerpt: "Eletrocussão prevenir: disjuntor diferencial 30mA obrigatório, não mexer quadro, água longe eletricidade. Descubra riscos, primeiros socorros e segurança elétrica."
keywords: ["eletrocussão", "choque elétrico", "segurança elétrica", "disjuntor diferencial", "Trás-os-Montes"]
---

# Eletrocussão Prevenir: Segurança Elétrica Casa e Trabalho

**Medo choques elétricos? Crianças em casa? Quer segurança máxima?** Eletrocussão **mata 10-20 pessoas/ano Portugal** - prevenível 90% casos. Disjuntor diferencial 30mA obrigatório, não mexer quadro elétrico, água longe eletricidade, tomadas protegidas. Neste guia crítico segurança, explicamos riscos eletrocussão, prevenção eficaz, primeiros socorros, normas e como garantir instalação 100% segura.

## ⚡ O Que É Eletrocussão

### Definição

**Eletrocussão**:
- Passagem corrente elétrica corpo
- Pode causar queimaduras
- Paragem cardíaca
- Danos nervosos
- **Morte**

### Como Acontece

**Circuito completo**:
- Toca parte energizada (fase)
- Corrente atravessa corpo
- Sai por terra/neutro
- Coração/cérebro afetados
- **Segundos podem matar**

### Correntes Perigosas

**Efeitos por intensidade**:
- **1 mA**: Sensação formigueiro
- **5 mA**: Dor, consegue largar
- **10-20 mA**: Contração muscular, não consegue largar
- **50 mA**: Dor intensa, dificuldade respirar
- **100 mA**: Fibrilação ventricular (coração para)
- **200 mA**: Paragem cardíaca, queimaduras graves
- **>1 A**: Morte quase certa

**Tempo exposição crítico**: Quanto mais tempo, maior risco morte

## 🚨 Riscos Principais Casa

### Quadro Elétrico

**Perigo**:
- Tensão 230V direta
- Múltiplos circuitos
- Partes energizadas expostas
- **Muito perigoso**

**Nunca**:
- ❌ Abrir quadro sem conhecimento
- ❌ Mexer disjuntores molhado
- ❌ Tocar barramentos
- ❌ Arranjar sem desligar geral

**Sempre**:
- ✅ Chamar eletricista certificado
- ✅ Manter fechado
- ✅ Crianças longe

### Tomadas e Fichas

**Perigo**:
- Tomadas danificadas
- Fichas partidas
- Fios expostos
- Água perto

**Prevenção**:
- Substituir tomadas danificadas
- Proteger tomadas (crianças)
- Não sobrecarregar
- Longe água

### Água e Eletricidade

**Combinação mortal**:
- Água conduz eletricidade
- Risco eletrocussão 10x maior
- Casa banho = zona crítica
- Cozinha = risco alto

**Regras**:
- Não usar aparelhos molhado
- Não tocar interruptores mãos molhadas
- Secador longe lavatório
- Aparelhos longe banheira

### Extensões e Cabos

**Perigo**:
- Isolamento danificado
- Sobrecarga
- Água
- Tropeçar

**Prevenção**:
- Verificar estado
- Não sobrecarregar
- Não passar debaixo tapetes
- Substituir se danificado

### Aparelhos Avariados

**Perigo**:
- Fuga corrente
- Choque ao tocar
- Curto-circuito
- Incêndio

**Sinais**:
- Choque leve ao tocar
- Aquece excessivo
- Cheiro queimado
- Faíscas

**Ação**: Desligar imediatamente, não usar, arranjar/substituir

## 🛡️ Prevenção Eletrocussão

### Disjuntor Diferencial (OBRIGATÓRIO)

**O que é**:
- Deteta fuga corrente
- Desliga em 0.03 segundos
- Sensibilidade 30mA
- **Salva vidas**

**Como funciona**:
- Compara corrente entrada/saída
- Se diferença >30mA = fuga
- Dispara imediatamente
- Corta corrente

**Obrigatório**:
- Todas instalações novas (desde 2006)
- Casas banho
- Cozinha
- Exterior
- Tomadas

**Custo**: €50-150 (instalado)
**Teste**: Mensal (botão teste)

### Tomadas com Terra

**Essencial**:
- 3º pino = terra
- Desvia fuga corrente
- Protege pessoas
- Obrigatório aparelhos metálicos

**Verificar**:
- Todas tomadas têm terra
- Terra funciona (teste)
- Ligação correta

**Custo**: €15-30/tomada (adicionar terra)

### Proteção Crianças

**Tomadas**:
- Protetores tomadas: €5-15 (10 unidades)
- Tomadas segurança (obturador): €8-15/unidade
- Cobrir todas acessíveis

**Quadro elétrico**:
- Fechadura
- Altura inacessível
- Educar perigo

**Cabos**:
- Esconder/proteger
- Não deixar acessíveis
- Aparelhos fora alcance

### Instalação Certificada

**Obrigatório**:
- Eletricista certificado
- Projeto elétrico
- Inspeção DGEG (>50m²)
- Certificado conformidade

**Normas**:
- RTIEBT (Regras Técnicas)
- Disjuntor diferencial 30mA
- Secção cabos adequada
- Quadro normalizado

**Multas**: €250-€44,890 se ilegal

### Manutenção Regular

**Anual**:
- Testar disjuntor diferencial
- Verificar tomadas
- Inspecionar cabos
- Apertar ligações quadro

**5 anos**:
- Inspeção completa
- Termografia (pontos quentes)
- Teste terra
- Atualizar se necessário

**Custo**: €80-200 (inspeção completa)

## 🏠 Zonas Críticas Casa

### Casa Banho

**Riscos**:
- Água + eletricidade
- Humidade constante
- Pés descalços (terra)
- **Zona mais perigosa**

**Regras**:
- Volume 0 (banheira): Só 12V
- Volume 1 (1m banheira): IPX4 mínimo
- Volume 2 (2m banheira): IPX3 mínimo
- Disjuntor diferencial 30mA obrigatório

**Proibido**:
- Tomadas perto banheira (<60cm)
- Interruptores volume 0-1
- Aparelhos não IPX4

**Permitido**:
- Tomadas >60cm banheira
- Iluminação 12V
- Exaustor IPX4

### Cozinha

**Riscos**:
- Água torneira
- Aparelhos potentes
- Gordura (conduz)
- Mãos molhadas

**Prevenção**:
- Tomadas longe lava-loiça (>30cm)
- Não tocar aparelhos molhado
- Secar mãos antes
- Disjuntor diferencial

### Exterior

**Riscos**:
- Chuva
- Humidade
- Terra direta
- **Risco muito alto**

**Obrigatório**:
- Tomadas IP65 (estanques)
- Disjuntor diferencial 30mA
- Cabos enterrados protegidos
- Quadro estanque

**Custo**: €30-80/tomada exterior

### Garagem/Cave

**Riscos**:
- Humidade
- Ferramentas metálicas
- Chão betão (conduz)

**Prevenção**:
- Disjuntor diferencial
- Tomadas IP44 mínimo
- Iluminação adequada
- Quadro protegido

## 🚑 Primeiros Socorros Eletrocussão

### Passo 1: Segurança (5 segundos)

**NUNCA tocar vítima diretamente**:
- Ainda pode estar energizada
- Você também leva choque
- **Desligar corrente primeiro**

**Ações**:
1. Desligar disjuntor geral
2. Ou desligar aparelho tomada
3. Se impossível: Afastar vítima com material isolante (madeira seca, plástico)
4. **Nunca metal ou água**

### Passo 2: Avaliar Vítima (10 segundos)

**Verificar**:
- Consciente?
- Respira?
- Pulso?
- Queimaduras?

### Passo 3: Ligar 112 (30 segundos)

**Sempre ligar**:
- Mesmo se consciente
- Eletrocussão pode causar problemas horas depois
- Arritmias cardíacas
- Danos internos

**Informar**:
- Eletrocussão
- Tensão (230V casa, alta tensão)
- Estado vítima
- Localização

### Passo 4: Reanimação (Se Necessário)

**Se não respira**:
1. Deitar costas superfície dura
2. Inclinar cabeça atrás (abrir via aérea)
3. 30 compressões peito (centro, 5cm fundo, 100-120/min)
4. 2 insuflações boca-a-boca
5. Repetir até ambulância chegar

**Se respira mas inconsciente**:
- Posição lateral segurança
- Vigiar respiração
- Não dar água/comida

### Passo 5: Tratar Queimaduras

**Queimaduras elétricas**:
- Arrefecer água fria (10-20 min)
- Não gelo
- Não pomadas
- Cobrir pano limpo
- Não rebentar bolhas

**Atenção**: Queimaduras elétricas podem ser profundas (não visível)

## 💰 Custos Segurança Elétrica

### Proteções Obrigatórias

**Disjuntor diferencial 30mA**: €50-150
**Terra instalação**: €200-800 (se não existe)
**Tomadas terra**: €15-30/unidade
**Inspeção DGEG**: €150-300

**Total instalação segura**: €500-1,500

### Proteções Adicionais

**Protetores tomadas crianças**: €5-15 (10 unidades)
**Tomadas segurança obturador**: €8-15/unidade
**Tomadas exterior IP65**: €30-80/unidade
**Quadro fechadura**: €20-50

### Manutenção

**Inspeção anual**: €80-150
**Inspeção completa (5 anos)**: €150-300
**Termografia**: €200-400

### ROI

**Investimento**: €500-1,500 (segurança completa)
**Benefício**: **VIDA**
**ROI**: Incalculável

## 📋 Normas e Legislação

### RTIEBT

**Regras Técnicas Instalações Elétricas**:
- Decreto-Lei 226/2005
- Atualizado regularmente
- Obrigatório cumprimento
- Fiscalização DGEG

### Disjuntor Diferencial

**Obrigatório**:
- Todas instalações novas (desde 2006)
- Sensibilidade 30mA
- Tempo disparo <0.03s
- Teste mensal

### Inspeção DGEG

**Obrigatória**:
- Instalações >50m²
- Instalações novas
- Alterações significativas
- Renovação 15 anos

**Custo**: €150-300
**Validade**: 15 anos

### Certificação

**Eletricista certificado**:
- Formação específica
- Seguro responsabilidade civil
- Certificado DGEG
- **Obrigatório**

**Multas sem certificação**:
- Particular: €250-€3,740
- Empresa: €2,500-€44,890

## 🏠 Situações Específicas Trás-os-Montes

### Casas Antigas

**Problemas comuns**:
- Sem terra
- Sem disjuntor diferencial
- Cabos antigos (tecido)
- Quadro obsoleto
- **Muito perigoso**

**Solução**:
- Renovação completa: €2,000-5,000
- Ou mínimo: Disjuntor diferencial + terra: €500-1,500

### Humidade Inverno

**Risco aumentado**:
- Paredes húmidas conduzem
- Chão molhado
- Mãos húmidas
- **Cuidado redobrado**

**Prevenção**:
- Disjuntor diferencial essencial
- Secar mãos sempre
- Não tocar aparelhos molhado

### Zona Rural

**Desafios**:
- Casas isoladas
- Ambulância longe (30-60 min)
- Instalações antigas
- **Prevenção crítica**

**Recomendações**:
1. Disjuntor diferencial obrigatório
2. Formação primeiros socorros
3. Inspeção regular
4. Atualizar instalação

### Trabalho Agrícola

**Riscos**:
- Máquinas elétricas
- Exterior (chuva)
- Bombas água
- Vedações elétricas

**Prevenção**:
- Disjuntor diferencial 30mA
- Tomadas IP65
- Cabos protegidos
- Manutenção regular

## 📞 Quando Chamar Eletricista

### Urgente (Imediato)

✅ Choque elétrico (mesmo leve)
✅ Faíscas
✅ Cheiro queimado
✅ Disjuntor dispara constantemente
✅ Aparelho dá choque

**Ação**: Desligar geral + chamar urgente

### Preventivo (1 Semana)

✅ Instalar disjuntor diferencial
✅ Adicionar terra
✅ Proteger tomadas crianças
✅ Inspeção segurança
✅ Atualizar instalação antiga

### Manutenção (Anual)

✅ Inspeção completa
✅ Testar proteções
✅ Verificar ligações
✅ Termografia

## ✅ Por Que Escolher 

### Especialização Segurança

- Certificação DGEG
- Formação segurança
- Equipamento teste profissional
- 10+ anos experiência

### Instalação Segura Certificada

- Disjuntor diferencial 30mA
- Terra normalizada
- Tomadas segurança
- Certificado conformidade
- Garantia 2 anos

### Inspeção Completa

- Teste disjuntor diferencial
- Medição terra
- Termografia
- Relatório detalhado
- Recomendações

### Conhecimento Local

- Experiência Trás-os-Montes
- Casas antigas especialidade
- Soluções adaptadas
- Preços justos

### Resposta Urgente

- **15-30 minutos** em Bragança, Mirandela, Macedo
- 24h/7d incluindo feriados
- Eletrocussão = prioridade máxima
- Equipamento disponível

## 🌍 Cobertura

Atendemos **86 cidades** em Trás-os-Montes:
- Bragança, Mirandela, Macedo de Cavaleiros
- Vila Real, Chaves, Vinhais
- Vimioso, Miranda do Douro, Mogadouro
- E mais 77 cidades

## 💬 Contacte-nos URGENTE

### Emergência Elétrica 24h
📞 **+351 932 321 892**
💬 **[WhatsApp Urgente](https://wa.me/351932321892?text=ELETROCUSSÃO%20URGENTE)**

**Choque elétrico = Emergência extrema. Resposta em 5 minutos. Chegada 15-30 minutos.**

---

## ❓ Perguntas Frequentes

### Como prevenir eletrocussão?
**Disjuntor diferencial 30mA** obrigatório (€50-150), tomadas terra, não mexer quadro, água longe eletricidade, protetores tomadas crianças. Instalação certificada. Previne 90% acidentes.

### Disjuntor diferencial é obrigatório?
**Sim**. Obrigatório todas instalações novas desde 2006. Sensibilidade 30mA, dispara 0.03s. Salva vidas. Multas €250-€44,890 se não tiver. Custo €50-150 instalado.

### O que fazer se choque elétrico?
**1)** Desligar corrente (disjuntor). **2)** NÃO tocar vítima antes. **3)** Ligar 112. **4)** Reanimação se necessário. **5)** Mesmo se consciente, hospital (arritmias podem surgir horas depois).

### Casa antiga sem terra, é perigoso?
**Muito perigoso**. Sem terra + sem disjuntor diferencial = risco eletrocussão alto. Solução: Adicionar terra + disjuntor diferencial €500-1,500. Ou renovação completa €2,000-5,000.

### Tomadas casa banho são permitidas?
**Sim** mas regras: >60cm banheira, disjuntor diferencial 30mA obrigatório, IPX4 mínimo. Proibido volume 0-1 (perto banheira). Normas RTIEBT. Eletricista certificado instala.

### Quanto custa instalação segura?
**€500-1,500**: Disjuntor diferencial (€50-150), terra (€200-800), tomadas terra (€15-30/unidade), inspeção (€150-300). ROI: VIDA. Previne 90% acidentes. Obrigatório legal.

---

**Eletrocussão mata. Disjuntor diferencial 30mA obrigatório. Instalação certificada salva vidas.**

📞 +351 932 321 892 | 💬 WhatsApp 24h | 🌐 eletricista-norte-reparos.pt
