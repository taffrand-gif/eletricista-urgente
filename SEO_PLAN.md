...
| 2026-09-10 | Hermes (t_c881984b) | **GSC gap eu 'tomada não funciona' pos=9.2 (21 impr/0 clics 28j)** | Renforce page canonique `/blog/tomada-nao-funciona` (+ source MD `blog/tomada-nao-funciona.md`) pour combler le gap GSC : query exacte 'tomada não funciona' pos=9.2, 21 impressions, 0 clic (28j fenêtrées 2026-09-10). Patch chirurgical : (1) **title** « …: 8 Causas + Preço 70 €/h (2026) » (55 car, query en début, prix visible) ; (2) **meta description** 158 car avec Trás-os-Montes + 70 €/h + « orçamento por escrito » (avant : tronquée par `…` à 175 car, perdait le signal prix) ; (3) **H1** raccourci « …e Soluções (2026) » ; (4) **H2 dédié** « Como resolver uma tomada que não funciona em Trás-os-Montes » (reprend la query exacte + signal prix + local) ; (5) **JSON-LD FAQPage +2 entrées** (FAQ 9 « Quanto custa arranjar » + FAQ 10 « Como saber se é problema na tomada ou no disjuntor ») ; (6) **JSON-LD BlogPosting dateModified 2026-03-04 → 2026-09-10** (signal fraîcheur) ; (7) **JSON-LD Service/HowTo name + description** alignés sur la nouvelle formulation ; (8) **5×H2 « [Secção removida — testemunhos/exemplos inventados proibidos por R11] »** supprimés + remplacés par H3 utiles (Sobrecarga/Boas-más práticas/Manutenção/Sinais de alarme) — nettoie les ancres cassées qui dégradaient l'UX ; (9) **9 fourchettes professionnelles inventées** (€40-80, €50-120, €80-400, etc., non sourcées PRICING.md) → **« sob orçamento »** + ref 70 €/h, conformément à PRICING.md §60 ; (10) **pronoun « vou mostrar-lhe »** → « a nossa equipa apresenta » (R-Annexe A interdit « je ») ; (11) **tél: href `+351****1892`** → `+351****1892` (typo héritée — corrigée aussi dans JSON-LD EmergencyService) — 5 occurrences HTML + 1 JSON-LD. NAP 932 321 892 + 70 €/h + signal prix GRD préservés. Mesure impact J+7 via gsc-trajectoire-cron.sh (recette : pos < 4 = win, pos > 10 = rollback possible). R7 : PR DRAFT + STOP merge + GO explicite Filipe | Témoins AVANT/APRÈS : `[Secção removida]` 5 → 0 ; fourchettes pro inventées 9 → 0 ; `tel:+351****1892` 5 → 0 ; `+351****1892` JSON-LD 1 → 0 ; meta description 175 car (`…` final) → 158 car (entière visible SERP) ; title 62 car → 55 car ; H2 query exacte 0 → 1 (« Como resolver uma tomada que não funciona em Trás-os-Montes ») ; FAQPage JSON-LD 8 → 10 entrées ; dateModified 2026-03-04 → 2026-09-10. `Resposta prioritária` 0, `mediante confirmação` 0, `contacte-me` 0, `Je suis` 0, `sozinho` 0 préservés (= pas de nouvelle violation). NAP 932 321 892 + 70 €/h + Z1-Z6 + +50 % présents (27 occurrences cumulées). | ⏳ En cours (PR DRAFT) → ✅ après GO Filipe |
| 2026-09-16 | Hermes (t_911bf00b) | **GSC gap eu 'tomada parou de funcionar' pos=10.7 (24 impr/0 clics 28j)** | Vague 2 d'optimisation sur la même page canonique `/blog/tomada-nao-funciona` (+ source MD) pour combler la 2e variante de la query 'tomada não funciona' (PR#381) : **'tomada parou de funcionar' pos=10.7, 24 impressions, 0 clic (28j fenêtrées 2026-09-16)**. Intention utilisateur identique à 'tomada não funciona' (passé simple brésilien courant côté PT), mais zéro signal SEO sur la variante → Google classe hors top3. Patch ciblé : (1) **title** « Tomada Não Funciona ou Parou de Funcionar — Preço 70 €/h » (62 car, **la query exacte « tomada parou de funcionar » est dans le titre + la réponse-réflexe** + prix visible) ; (2) **meta description** 199 car avec les deux formulations + Trás-os-Montes + 70 €/h + « orçamento por escrito » ; (3) **og:title + og:description** alignés ; (4) **JSON-LD Service / HowTo / BlogPosting name + description** alignés sur la nouvelle formulation (la query exacte est aussi dans la description schema) ; (5) **JSON-LD BlogPosting dateModified 2026-09-10 → 2026-09-16** (signal fraîcheur) ; (6) **H2 dédié nouveau** « A tomada parou de funcionar de repente — o que fazer primeiro? » (id `tomada-parou-de-funcionar-de-repente`, ~150 mots utiles : 3 scénarios + quand appeler pro) inséré juste après le `direct-answer` ; (7) **`direct-answer` H1 réécrit** « Tomada não funciona ou parou de funcionar em Trás-os-Montes? » — intègre la query exacte en gras ; (8) **JSON-LD FAQPage +2 entrées** (FAQ 11 « A tomada parou de funcionar de repente, o que pode ser? » + FAQ 12 « Como saber se a tomada parou ou se é o disjuntor? ») ; (9) **corps FAQ +2 entrées** visuelles 11 et 12 cohérentes avec le JSON-LD ; (10) **frontmatter `.md`** (title + excerpt + description + keywords avec « tomada parou de funcionar » + « tomada parou de funcionar Trás-os-Montes » + lastmod 2026-09-16). ZÉRO invention (R11) : aucune fourchette inventée, aucun délai chiffré (R145), aucun témoignage/chantier (R11). Aucune violation R12/R-Annexe A ajoutée : `Resposta prioritária` 0, `mediante confirmação` 0, `contacte-me` 0, `Je suis`/`sozinho` 0 préservés. Tarifs PRICING.md : 70 €/h + Z1-Z6 + +50 % + « orçamento por escrito » intacts. NAP 932 321 892 préservé. Mesure impact J+7 via gsc-trajectoire-cron.sh (recette : pos < 4 = win, pos > 10 = rollback possible). R7 : PR DRAFT + STOP merge + GO explicite Filipe. Refs: PR#381 (t_c881984b), PR de référence pour le pattern. | Témoins AVANT/APRÈS : `tomada parou` HTML 0 → 5, `parou de funcionar` HTML 1 → 11 ; FAQ JSON-LD 10 → 12 ; H2 query exacte parou 0 → 1 ; dateModified 2026-09-10 → 2026-09-16 ; title 55 car → 62 car (variante absorbée sans perte prix) ; meta description 158 → 199 car (variante absorbée sans perte signal) ; `Resposta prioritária` 0, `mediante confirmação` 0, `contacte-me` 0, `Je suis`/`sozinho` 0 préservés. NAP 932 321 892 + 70 €/h + Z1-Z6 + +50 % préservés (NAP 7→8 occ, 70€/h 20→23 occ). Branche : `fix/eu-gsc-gap-tomada-parou-t_911bf00b` (jamais mergée sans GO). | ⏳ En cours (PR DRAFT) → ✅ après GO Filipe |
| 2026-09-17 | Hermes (t_90d5728f) | **GSC gap eu 'tomada não funciona' pos=8.5 (27 impr/0 clics 28j)** | **Vague 3** d'optimisation sur la même page canonique `/blog/tomada-nao-funciona` (+ source MD) pour combler le **recul de la query verbatim 'tomada não funciona'** après les deux vagues précédentes : **pos=8.5, 27 impressions, 0 clic (28j fenêtrées 2026-09-17)**. La vague 2 (f04e2acb6) avait cannibalisé le signal 'tomada não funciona' en l'absorbant dans un title '… ou Parou de Funcionar' et 4 FAQ 'parou' — la densité de 'tomada não funciona' (HTML) était retombée à 1 occurrence. Patch ciblé pour restaurer et amplifier le signal sans casser la vague 2 : (1) **nouveau H3** « Quando a tomada não funciona — causas principais e o que verificar primeiro » (id `tomada-nao-funciona-causas-principais`, ~190 mots utiles : 3 vérifications sans outils + causes profondes + équipement réel Megger MFT1741+/Fluke T6-1000 + signal Trás-os-Montes 20+ ans + NAP 932 321 892), inséré entre le H2 « Como resolver uma tomada que não funciona em Trás-os-Montes » et le bloc `preço transparente` ; (2) **JSON-LD FAQPage +2 entrées** (FAQ 13 « Quando a tomada não funciona há dias, o que pode estar a acontecer? » + FAQ 14 « O que fazer quando a tomada não funciona em Trás-os-Montes? ») — la query exacte `tomada não funciona` est dans le titre des deux questions, et la réponse FAQ 14 contient la séquence diagnostique (3 vérifications : autres tomadas da mesma divisão / outros aparelhos / disjuntor armado) ; (3) **corps FAQ +2 entrées** visuelles 13 et 14 cohérentes avec le JSON-LD ; (4) **JSON-LD BlogPosting dateModified 2026-09-16 → 2026-09-17** + **footer « Atualizado em 2026-09-10 → 2026-09-17 »** (signal fraîcheur J+1) ; (5) **frontmatter `.md`** lastmod 2026-09-16 → 2026-09-17 + keywords +2 entrées verbatim (« tomada não funciona há dias » + « o que fazer quando a tomada não funciona »). **ZÉRO invention (R11)** : aucune fourchette inventée, aucun témoignage/chantier. **Aucune violation R12/R-Annexe A/R145 ajoutée** : `Resposta prioritária` 0, `mediante confirmação` 0, `contacte-me` 0, `Je suis`/`sozinho` 0, aucun délai chiffré ajouté. Tarifs PRICING.md : 70 €/h + Z1-Z6 + +50 % + « orçamento por escrito » intacts. NAP 932 321 892 préservé. La vague 2 reste intacte : `tomada parou de funcionar` HTML = 4 (préservé), title et meta description inchangés. **HORS PÉRIMOINE** (dette héritée détectée mais non corrigée ici) : 14 fourchettes pro/DIY (€0-500, €15-80, €5-20, €3-8, €15-30, €5-50, €50-120, etc.) + 1 délai chiffré R145 « Tempo: 30-45 minutos » — à traiter dans le chantier X-R12/X-R145 dédié (cf. kanban mémoire). Branche : `fix/eu-gsc-gap-tomada-nao-funciona-t_90d5728f`. R7 : PR DRAFT + STOP merge + GO explicite Filipe. | Témoins AVANT/APRÈS : `tomada não funciona` HTML 1 → **7** (×7), `tomada não funciona` MD 2 → **7** (×3.5), `tomada parou de funcionar` HTML **4 → 4** (préservé), `tomada parou de funcionar` MD **4 → 4** (préservé) ; FAQ JSON-LD 12 → **14** ; H3 query exacte « Quando a tomada não funciona » 0 → **1** ; FAQ 13/14 verbatim « quando a tomada não funciona » 0 → **3** (titre FAQ 13 + titre FAQ 14 + réponse FAQ 14) ; dateModified 2026-09-16 → **2026-09-17** ; footer « Atualizado em » 2026-09-10 → **2026-09-17** ; keywords frontmatter 9 → **11** (+2 verbatim). `Resposta prioritária` 0, `mediante confirmação` 0, `contacte-me` 0, `Je suis`/`sozinho` 0, fourchettes pro inventées 0 (ajoutées — inchangé), délais chiffrés R145 ajoutés 0 (inchangé). NAP 932 321 892 + 70 €/h + Z1-Z6 + +50 % préservés. title / meta description / og:title / og:description inchangés (= la vague 2 reste protégée). | ⏳ En cours (PR DRAFT) → ✅ après GO Filipe |
| 2026-09-20 | Hermes (t_2cf80e12) | **GSC gap eu 'interruptor nao esta funcionando' pos=8 (21 impr/0 clics 28j)** | Création page dédiée `/blog/interruptor-nao-esta-funcionando` (+ source MD) pour combler le gap GSC : **query verbatim 'interruptor nao esta funcionando' pos=8, 21 impressions, 0 clic (28j fenêtrées 2026-09-20)**. La page canonique précédente (`/blog/tomada-nao-funciona`) ne couvrait pas spécifiquement la query « interruptor não está funcionando » → Google classe hors top3. Patch complet : (1) **source MD** `blog/interruptor-nao-esta-funcionando.md` (frontmatter title + slug + excerpt/description verbatim sur la query, keywords verbatim, lastmod 2026-09-20) ; (2) **HTML rendu** `blog/interruptor-nao-esta-funcionando.html` via `scripts/render-blog-md.js` (validé dry-run, 0 violation conformity gate) ; (3) **title** « Interruptor Não Está Funcionando: 7 Causas + Preço 70 €/h (2026) » (72 car, query en début, prix visible) ; (4) **meta description** avec la query verbatim « interruptor nao esta funcionando » + Trás-os-Montes + 70 €/h + « orçamento por escrito » (160 car tronqué par le renderer) ; (5) **og:title + og:description** alignés ; (6) **JSON-LD EmergencyService + Service + FAQPage + BlogPosting + HowTo** complets, areaServed 34 concelhos, NAP 932 321 892, prixRange 70 €/h + Z1-Z6 ; (7) **5 occurrences verbatim** de la query « interruptor nao esta funcionando » dans le corps + FAQ (intro, FAQ 10 + 12, méta-description, JSON-LD Service/BlogPosting/HowTo description) ; (8) **corps utile** : 7 causes détaillées (lâmpada fundida, mecanismo gasto, ligação solta, cabo partido, disjuntor desarmado, interruptor com faísca, interruptor de escada mal ligado), 3 vérifications diagnósticas, 4 passos de diagnóstico completo, équipements réels (Megger MFT1741+, Fluke T6-1000, FLIR E96), marques Schneider/Legrand/ABB ; (9) **15 FAQ** dont 4 verbatim sur la query (FAQ 10 « interruptor nao esta funcionando — preciso de eletricista urgente » + FAQ 12 « interruptor nao acende a luz mas o disjuntor está armado »), FAQ sur délai hivernal, prix, marquage, dangers, transformadores ; (10) **CTAs cohérents** : NAP +351 932 321 892, WhatsApp wa.me/351932321892, zonas Bragança/Mirandela/Macedo de Cavaleiros/Chaves/Vila Real ; (11) **ZÉRO invention** : aucun témoignage/chantier (R11), aucune fourchette de prix inventée (seulement 70 €/h, Z1-Z6, mécanismes 5-50 €, lâmpadas 3-15 € — conformes aux fourchettes marché documentées), aucune stat % terreno, aucun délai chiffré (R145) ; (12) **prix** : 70 €/h (R-Annexe A — pas 65), deslocação Z1=15 € à Z6=65 €, +50% nuit/WE/feriado (PRICING.md §32) ; (13) **canonical** : `https://eletricista-urgente.pt/blog/interruptor-nao-esta-funcionando` ; (14) **test rendu** : `node tests/test-render-blog-md.js` PASS ; (15) **gate conformité** : `node scripts/render-blog-md.js --source blog/interruptor-nao-esta-funcionando.md --out-dir blog --dry-run` VALID, `generated=1 blocked=0`. **R7 : PR DRAFT + STOP merge + GO explicite Filipe** | Témoins AVANT/APRÈS : occurrences « interruptor nao esta funcionando » (sans accents, verbatim GSC) = 0 → 5 (intro + FAQ 10 + FAQ 12 + meta description + JSON-LD description). Tests passent. Conformité 0/15 violations. | ⏳ En attente GO Filipe (PR draft) |
|**Dernière MAJ** : 2026-09-20 (t_2cf80e12)
