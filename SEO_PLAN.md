...
|---|---|---|---|---|---|---|
| 2026-08-05 | Hermes (Kanban `t_bc868eec`) | **[CONFORMIDADE-URGENT] eu : scope-electric-on-plumbing em eletricista-avaria-eletrica-ribeira-de-pena.html** | Fix de conformidade unitário sobre `eletricista-avaria-eletrica-ribeira-de-pena.html` (Ribeira de Pena = Z6/65€, 130 km autour de Macedo de Cavaleiros). **FPP scope-electric-on-plumbing réfuté** (page 100% élec, 4 occurrences `canalizador` = `sameAs` JSON-LD Annexe A backlinks conformes Norte-OS, aucun service plomberie listé). **R145 purge (5 chaînes)** : `atendimento mediante confirmação por telefone` (x3) → `Atendimento 24h/7d após contacto telefónico` / `24h/7d, após contacto telefónico` ; `Diagnóstico após confirmação por telefone + reparação` → `Diagnóstico e reparação urgentes` ; `confirmação prévia` → `marcação prévia`. **Z6/65€ source-of-truth align** : badge `data-zone="6"`, `Deslocação Zona 6: 65€` (já incluída), grille Doctrine §12 Z1=15 €...Z6=65 € intacte. Branche `fix/eu-conform-ribeira-pena-scope-r145-z6-t_bc868eec` depuis `origin/main@955749ea0`, 1 commit `277fcbd1b`, **PR DRAFT #248** `state=OPEN isDraft=true`. 1 fichier, +1/-1. | DFSEO source-of-truth (`zonas-data.json['Ribeira de Pena'] = 6`) + AGENTS.md §12 R12 Transparence Radicale (70 €/h élec, +50 %, orçamento por escrito) + R145 (délai chiffré INTERDIT, `24h/7d` OK, `mediante confirmação` BANNIS). Pattern identique à t_79133bc0 (moucos), t_7ec530ae (cumieira), t_8548d01c (sanfins), t_6ad41a8f (valdigem), t_11af8ff5 (xinzo). R7 : PR DRAFT, **zéro merge sans GO nominatif Philippe**. Aucune invention de prix/zone/délai/service. | PR DRAFT #248 ouverte, `git diff --check` vert, scope strict 1 fichier, **0 hit R12/R145 INTERDIT** post-fix. | ⏳ PR #248 — attente GO nominatif Philippe avant merge |
