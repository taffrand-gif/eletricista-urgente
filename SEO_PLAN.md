...
| 2026-09-24 | Hermes (t_7a7c928a) | **[CONFORMIDADE-URGENT] eu wrong-phone eletricista-aguas-frias — KO RÉFUTÉ (close-on-arrival, scan faux-positif)** | Brief `t_7a7c928a` (scan local « wrong-phone » sur `eletricista-aguas-frias.html`, extrait `ign:center;background:#0a4d68;color:#fff…` non vérifié sur disque) = **même classe de faux-positifs** que `t_7b290488` (eu/aguas-vivas), `t_c6432a28` (cnr/salzedas), `t_ac479308` (cnr/marialva), `t_1662f744` (cu/alvendre), `t_c0beada0` (eu/amendoeira), `t_4fa3d40f` (cu/almendra), `t_9ce7c4c5-mezquita-cu` : signal `HTTP code = None` ou « wrong-phone » détecté en local, réfuté en HTTP 200 stable et NAP conforme. **Vérifications live (R02)** à l'instant t : (a) `sha256 local = sha256 live` (`a4cf1e966d9011702306aa6e8898482bb7c570f17e24aa5aa38fefbe0a8723d4`) ; (b) téléphone exhaustif : `+351 932 321 892` = 8, `932 321 892` forme courte = 10, `+351-932 321 892` (JSON-LD) = 1, `tel:+351****1892` (tel: href masqué cf. PRICING.md §NAP) = **5**, `wa.me/351932321892` = **6**, **928 = 0**, **928 484 451 = 0**, **autres 9 chiffres = 0** — 30 occurrences total, 100 % NAP élec conforme ; (c) ETag Vercel `b0ccfbfd6c9693a35b41b57e99a1eb79` stable depuis 14/09 (cf. audit SMKO 14/09, 5/5 probes HTTP 200) ; (d) **extrait cité partiellement reconstitué** : 0 hit `ign:center;background:#0a4d68` (le CSS réel est `align:center`, pas `ign:center` — copier-coller approximatif) + 0 hit `color:#fff"><h2 style="color:#fff">Eletricista` + 0 hit `Eletricista em Águas Frias?</h2>` (le H2 réel est sans `?` final). Les fragments individuels (`class="cta"`, `tel:+351****1892`, `#0a4d68`) existent bien dans le fichier (5 occurrences chacun), mais leur concaténation dans le contexte cité + H2 « Águas Frias? » = composite fabriqué ; (e) **absence contamination plomberie** : 0 hit `Fuga ativa`/`Inundação`/`Cano rebentado`/`Esgoto a transbordar`/`Válvula de segurança`/`feche a torneira geral`/`feche o contador` — le fix générique `e24f1cb00` (corticos) ne s'applique pas ici ; (f) JSON-LD `LocalBusiness.telephone = "+351-932 321 892"` ✅ + `sameAs` pointe vers sites irmãos (légitime R12 Annexe A) ; (g) `<title>` = « Eletricista Profissional em Aguas Frias — Norte Reparos », `<h1>` = « Eletricista Profissional em Aguas Frias ». **Décisions worker `t_7a7c928a`** : (1) **PAS de patch wrong-phone** — fichier déjà conforme (cf. rapport `_audit/SMKO-eu-2026-09-24-aguas-frias.md`) ; (2) **PAS de correction template-bleed** — contamination plomberie absente ; (3) **PAS de merge** — R7 « 0 merge sans GO » + « aucun prix, zone, délai ou service ne doit être inventé » (cf. brief) ; (4) rapport `_audit/SMKO-eu-2026-09-24-aguas-frias.md` créé (9038 octets, sha256 `9a6bd555d40649b758bd539214f6cc2413d5bc08ed66ee4249f96c27438b6651`) consignant la réfutation et la leçon-série (9e KO signalisé réfuté en 11 jours sur les 4 sites eu/cu/cnr) ; (5) **PR draft** `fix/eu-conform-aguas-frias-wrong-phone-t_7a7c928a` à ouvrir pour formaliser la consignation dans le repo GitHub et notifier Filipe. Défauts **secondaires** détectés (hors-porte wrong-phone) : `<h2>` = « Eletricista em Águas Frias » sans mot-clé d'intention type « urgente »/« 24h »/« preço » — défaut stylistique mineur, à batcher avec GO explicite selon AGENTS.md §12 R12 §validation « AUCUN batch sans prototype 1 page test ». **Prochaine action worker** : PR draft consignation (2 fichiers : `SEO_PLAN.md` + `_audit/SMKO-eu-2026-09-24-aguas-frias.md`), pas de push sans GO Filipe (R3 + R7). | ✅ Fait (rapport créé + PR draft à ouvrir) |
**Dernière MAJ** : 2026-09-24 (t_7a7c928a)
