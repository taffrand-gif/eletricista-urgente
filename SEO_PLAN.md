...
| 2026-09-21 | Hermes (t_7141ea38) | **GSC gap eu 'luz nao acende' pos=10.3 (20 impr/0 clics 28j)** | Création page dédiée `/blog/luz-nao-acende` (+ source MD) pour combler le gap GSC : **query verbatim 'luz nao acende' pos=10.3, 20 impressions, 0 clic (28j fenêtrées 2026-09-21)**. Les pages canoniques précédentes (`/blog/sem-luz-casa-toda`, `/blog/sem-luz-parcial-urgencia`, `/blog/luz-pisca-causas-resolver`, `/blog/tomada-nao-funciona`) ne ciblaient pas spécifiquement la query « luz nao acende » → Google classe hors top3. Patch complet aligné sur le pattern validé t_2cf80e12 (page `interruptor-nao-esta-funcionando`) : (1) **source MD** `blog/luz-nao-acende.md` (frontmatter title + slug + excerpt/description verbatim sur la query, keywords verbatim, lastmod 2026-09-21) ; (2) **HTML rendu** `blog/luz-nao-acende.html` via `scripts/render-blog-md.js` (validé dry-run, 0 violation conformity gate) ; (3) **title** « Luz Não Acende: 7 Causas + Preço 70 €/h (2026) » (52 car, query en début, prix visible) ; (4) **meta description** avec la query verbatim « luz nao acende » + Trás-os-Montes + 70 €/h + « orçamento por escrito » (160 car tronqué par le renderer) ; (5) **og:title + og:description** alignés ; (6) **JSON-LD EmergencyService + Service + FAQPage + BlogPosting + HowTo** complets, areaServed 34 concelhos, NAP 932 321 892, prixRange 70 €/h + Z1-Z6 ; (7) **6 occurrences verbatim** de la query « luz nao acende » (sans accents, verbatim GSC) + **5 occurrences** « luz não acende » (avec accents, variante PT-PT) — meta description + JSON-LD Service/FAQPage/BlogPosting/HowTo + direct-answer + intro + FAQ 1/10/12 + H2 « Luz não acende — o que verificar primeiro » + H2 « Como resolver uma luz que não acende em Trás-os-Montes » + H3 « 🚨 Luz Não Acende? Ajuda Profissional em Trás-os-Montes » ; (8) **corps utile** : 7 causes détaillées (lâmpada fundida, mecanismo do interruptor gasto, casquilho queimado, ligação solta na caixa, cabo partido dentro da parede, disjuntor desarmado, interruptor com faísca/aquecimento), 3 vérifications diagnósticas, 4 passos de diagnóstico completo, équipements réels (Megger MFT1741+, Fluke T6-1000, FLIR E96), marques Schneider/Legrand/ABB ; (9) **15 FAQ** dont **3 verbatim** sur la query (FAQ 1 « A luz nao acende mas o disjuntor está armado — o que fazer? » + FAQ 10 « Luz nao acende em parte da casa — preciso de eletricista urgente? » + FAQ 12 « Por que é que a luz nao acende mesmo com lâmpada nova? ») + FAQ sur prix, délai hivernal, dangers, certification, casquilho, comutador de escada, mécanisme certificado ; (10) **CTAs cohérents** : NAP +351 932 321 892, WhatsApp wa.me/351932321892, zonas Bragança/Mirandela/Macedo de Cavaleiros/Chaves/Vila Real ; (11) **ZÉRO invention** : aucun témoignage/chantier (R11), aucune fourchette de prix inventée (seulement 70 €/h, Z1-Z6, mecanismos 5-50 €, lâmpadas 3-15 € — fourchettes marché documentées), aucune stat % terreno, aucun délai chiffré (R145), aucun pronom « je/minha » (R-Annexe A) ; (12) **prix** : 70 €/h (R-Annexe A — pas 65), deslocação Z1=15 € à Z6=65 €, +50% nuit/WE/feriado (PRICING.md §32) ; (13) **canonical** : `https://eletricista-urgente.pt/blog/luz-nao-acende` ; (14) **test rendu** : `node tests/test-render-blog-md.js` PASS ; (15) **gate conformité** : `node scripts/render-blog-md.js --source blog/luz-nao-acende.md --out-dir blog --dry-run` VALID `generated=1 blocked=0` + audit sur HTML rendu = `findings=0` (PASS) ; (16) **CLEANUP pollution** : 7 parens orphelins `()` après titres Causa 1-7 retirés du MD (artefacts copier-coller), 2 occurrences CJK `蜂窝` (« nid d'abeilles » en chinois) → `ninhos de vespas` (PT-PT cohérent — leçon X-CJK PR #397) ; (17) **maillage interne** : 5 liens vers pages connexes (`/blog/tomada-nao-funciona`, `/blog/interruptor-nao-esta-funcionando`, `/blog/disjuntor-desarma`, `/blog/casa-sem-luz-o-que-fazer`, `/eletricista-braganca`). **R7 : PR DRAFT + STOP merge + GO explicite Filipe** | Témoins AVANT/APRÈS : occurrences « luz nao acende » (sans accents, verbatim GSC) = 0 → 6 (intro + direct-answer + FAQ 1 + FAQ 10 + FAQ 12 + JSON-LD description + meta description) ; occurrences « luz não acende » (avec accents) = 0 → 5 dans le corps + 5 dans title/H1/H2 (total visible SERP ≈ 5× par blocs canoniques) ; `70 €/h` HTML = 44 ; `932 321 892` HTML = 15 ; `Megger MFT1741+\|Fluke T6-1000\|FLIR E96` HTML = 12 ; `orçamento por escrito` HTML = 24 ; `application/ld+json` blocks = 5 ; @type présents : EmergencyService + Service + FAQPage + BlogPosting + HowTo (+ Answer/Question/HowToStep/Offer/Organization/AdministrativeArea) ; FAQ JSON-LD = 15 entrées (3 verbatim) ; pollution cleanup : `\(\)` 7 → 0, `蜂窝` 2 → 0. Tests passent (`render-blog-md tests: PASS`). Conformité 0/15 violations. Branche : `fix/eu-gsc-gap-luz-nao-acende-t_7141ea38` (depuis origin/main, jamais mergée sans GO). | ⏳ En attente GO Filipe (PR draft) |
|**Dernière MAJ** : 2026-09-21 (t_7141ea38)
