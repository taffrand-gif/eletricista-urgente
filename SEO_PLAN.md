...
**Rappel AGENTS.md §12 R145 doctrine** : aucun délai chiffré `em X min` / `X horas` / `24h/7d` autorisé en -urgente. Seules formulations acceptées : « Sob marcação », « Atendimento », « conforme disponibilidade ».
...
## 🔄 HISTORIQUE
| 2026-08-29 | cowork-loop | **Rayon de couverture — 8 pages annonçaient `raio 100 km`, dont 7 articles de blog jamais atteints par les batchs de rayon** | EU était **le repo le plus atteint des quatre** : **8 fichiers** contre 3 (CNR), 3 (ENR) et 1 (CU), pour **32 fichiers seulement** portant déjà `130` — le taux de contradiction interne y est d'un ordre de grandeur supérieur. Les 8 se répartissent en deux gabarits : (a) **7 articles `public/blog/`** avec la formule de couverture régionale en fin d'article (`num raio de 100 km`, `(raio 100km)`) ; (b) **1 article « 24h premium »** avec le `<h2>` `Zonas Atendidas (Raio 100km)` — **le même gabarit qu'au jumeau CU**, traité au même run. 🔎 **Le sous-arbre `public/blog/` est un angle mort récurrent** : les correctifs de rayon des 4 repos ont tous porté sur les pages de service et le JSON-LD ; **aucun n'a inclus les articles de blog**, qui portent pourtant la même promesse de couverture. ⚠️ **Non touché délibérément** : `tools/enrich_concelhos.py` et `guia-eletricista-tras-os-montes.html` (**bornes de zone**). 🔴 **Anomalie signalée, non corrigée ce run** : `public/blog/certificacao-eletrica-preco.html` sert `(raio 130km</li>` — **la parenthèse ouvrante n'est jamais fermée**. Le défaut préexiste au patch et relève d'une **autre famille** (ponctuation) ; le corriger ici aurait mêlé deux familles dans un même commit. **À traiter au prochain run.** | R4 (zéro invention — valeur `130 km` verrouillée par `AGENTS.md` « rayon ~130 km autour de Macedo de Cavaleiros » et `PRICING.md` « Z6 = ~130 km route max », et déjà majoritaire en production), R11/R12 (contradiction active en production = priorité sur la tâche prévue), R8 (témoin avant/après, 1 motif par commande, compté en Python sur le contenu des fichiers), commit atomique 1 fichier = 1 commit, R6 (zéro force-push), R7 (zéro merge), R-WT (aucune copie de travail partagée touchée — commits construits par plumbing `commit-tree` sur un index temporaire, ni branche courante ni index ni fichier du checkout partagé modifiés) | **9 commits, 8 fichiers de production** + `SEO_PLAN.md`. **Témoin R8** — périmètre : arbre complet de `origin/main` hors `_archive/ _audit/ _backlog/ docs/ node_modules/`, hors `.md` = **2486 fichiers** ; motif `[Rr]aio[^.<]{0,18}100` : **8 → 0** ; motif `…130` : **24 → 32 fichiers** ; scan détagué de rattrapage : **0**. `git diff --numstat` = **1/1 sur chacun des 8 fichiers**. **Diff octet à octet : exactement 1 octet par fichier, `0` → `3` — 8 octets modifiés au total sur 8 fichiers.** Delta de balises **vide sur les 8**. Mots : `100km`→`130km` ou `100`→`130`, **aucun autre mot ajouté ni retiré sur aucun fichier**. Branche `loop/2026-08-29-eu-raio-130km` depuis `origin/main` (`e7ce215a3`). | ⏳ branche poussée, PR à ouvrir |
| 2026-08-27 | cowork-loop | **Rang 2(b) — le CTA WhatsApp d'un article de blog était un lien Markdown servi en TEXTE, donc non cliquable ; + rang 4 — les 3 dernières pages de la famille `<section>` 6/5 ; + une SECONDE anomalie sur `eletricista-urgente-mirandela.html` ; + rang 10 livré** | 🟢 **Contrôle d'ouverture : `gh pr list --state open` rend 0 PR ouverte sur les 4 repos.** Sur EU, les 4 PR (#308, #311, #313, #317) ont mergé — les rangs 4, 5, 6 et 7 étaient **bloqués mécaniquement** par ce stock, ils sont libres. **Aucun GO consommé ce run.** **Rang 2(b)** : `[WhatsApp](https://wa.me/351932321892?text=…)` était servi tel quel au visiteur dans `public/blog/blog-problemas-eletricos-inverno.html` — **un CTA de money page non cliquable**, résidu du convertisseur Markdown traité par #317. Balayage du prédicat « lien Markdown `[texte](url)` dans un fichier NON-`.md` » sur les **2 573** fichiers hors `_archive*` : **7 occurrences / 2 fichiers**, dont **6 dans `llms.txt`** — où le Markdown est **le format attendu** (spécification llms.txt) et n'est donc **pas** un défaut. **Il ne restait qu'UNE occurrence réelle.** Transformation `[X](U)` → `<a href="U">X</a>` : **le libellé et l'URL viennent tous deux, verbatim, de la chaîne cassée elle-même** — zéro invention. **Rang 4** : `eletricista-urgente-mirandela.html`, `-miranda-do-douro.html`, `-alfandega-da-fe.html` portaient un `<section class="unique-urg-*">` jamais fermé avant l'ouverture du suivant. **Invariant mesuré** : la profondeur `<section>` avant un `<section class="unique-urg-*">` vaut **0 sur 154 des 157 occurrences du dépôt** ; les **3 exceptions sont exactement ces 3 fichiers**. Correctif identique à #316 (mergée) : insertion de `</section>`. 🔴 **SECONDE ANOMALIE, famille différente, sur le même fichier** : `eletricista-urgente-mirandela.html` restait à `<div>` **−1** après la réparation du `<section>`. Diagnostic par **diff de la séquence des balises `<div>`** contre un jumeau sain (`-miranda-do-douro`) : **une seule différence, un `<div class="wrap">` manquant en 5ᵉ position**. **Population** : sur les **77** pages `eletricista-urgente-*` portant le motif de fin `</div></div></div><div class="cta-bottom">`, **76 portent `</p></div><div class="wrap">`** et **mirandela est la seule à ne pas le porter — et la seule déséquilibrée**. 76 donneurs, une seule forme. | R4 (zéro invention — libellé et URL transplantés de la chaîne cassée ; `</section>` et `<div class="wrap">` transplantés de donneurs à forme unique ; **zéro mot ajouté ou retiré sur les 4 fichiers**, contrôle par diff d'ensembles de mots vide dans les deux sens), R11/R12 (CTA mort et déséquilibres actifs en production = priorité), R8 (témoin avant/après par fichier, 1 motif, compté **en Python sur le contenu des fichiers**), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree `~/work/Sites/_loop-2026-08-27/wt-eu` ; checkout partagé — qui est **sur `main`** — **non touché**) | **5 commits, 4 fichiers de production** + `SEO_PLAN.md`. **Témoins R8** — liens Markdown bruts hors `.md` : **1 → 0** sur le blog (et 6 conservés dans `llms.txt`, hors périmètre). `<section>` : **+1 → 0** sur les 3 pages ; violations de l'invariant de profondeur **3 → 0**, soit **157/157**. `<div>` sur mirandela : **−1 → 0**, et sa séquence de balises `<div>` devient **identique élément par élément** à celle de `-miranda-do-douro` (96 = 96). **Contrôle « mot inexistant »** : ensembles de mots avant/après **strictement identiques sur les 4 fichiers** — corrections purement structurelles. **Delta de balises** : `a` +2 sur le blog (le lien devient un élément), `section` +1, `div` +1. 🔴 **RANG 10 LIVRÉ — et il INFIRME l'extrapolation depuis CNR, pour la seconde fois.** Périmètre énoncé : tout le dépôt hors `_archive*`, hors `_audit/ _backlog/ docs/`, hors les 6 `.md` de doctrine racine, **`blog/*.md` INCLUS** = **2 542 fichiers** ; fenêtre de **60 caractères**. **`orçamento`↔`gratuit` : 181 occurrences / 144 fichiers** — contre **110 / 100 sur CU**, **3 700 / 1 678 sur ENR** et **4 701 / 2 037 sur CNR**. 🔴 **Le clivage n'est pas aléatoire : il sépare les deux sites d'INSTALLATION (CNR, ENR : ~4 000 occurrences chacun) des deux sites d'URGENCE (CU, EU : ~150 chacun), soit un facteur ~30.** Ce ne sont pas les mêmes chaînes de génération. Autres mesures au même périmètre : `Atendimento 24h` **2 439 / 1 293** · `gratuit*` toutes formes **227 / 160** · `Você` **42 / 22** (dont 18 en `.md`) · corruption `*Parranj*` **25 / 16** (dont 8 en `.md`) · `raio de 100 km` **5 / 5** · `30/40 % dos incêndios` **17 / 4**. ⚠️ **40 des 56 `.md` ont un jumeau `.html`** : ce sont des sources de génération, les occurrences qui y vivent sont **latentes**, pas closes. Branche `loop/2026-08-27-eu-cta-whatsapp-mort` depuis `origin/main` (`bc0a124c9`). | ⏳ PR ouverte |
| 2026-08-26 | Hermes (Kanban `t_692a3266`) | **NO-OP APPLICABLE 5ᵉ passe — chantier défaut n°5 « 56 `<strong>` non fermés sur `blog-problemas-eletricos-inverno.html` » = DÉJÀ PRIS par PR #329 DRAFT (t_b53686cc, commit `6d64d3dd9`), zéro ouverture nouvelle** | Brief désignait « ligne 251 = 🔴/⏸/🛑 chantier vivant défaut n°5 du chantier Markdown non converti » + cite t_b53686cc consignation « OUVERTURE PR #329 DRAFT — patch défaut n°5 appliqué ». **Re-vérification live AVANT action (R04 stricte) sur 5 angles**, worktree `loop-eu-20260826-noop-l251-t_692a3266` = origin/main `6dae8ea14` (PR #328 mergée 14:03:02Z) : (1) `git log -1 --format='%H %s' origin/main` = `6dae8ea14` ✅ (PR #328 = NO-OP APPLICABLE ligne 251 amarante/alijo/penedono ✅ CLOS PR #316) ; (2) `gh pr view 329 --json state,isDraft,mergedAt,mergeable,mergeStateStatus,additions,deletions` = **`state=OPEN` `isDraft=true` `mergedAt=null` `mergeable=MERGEABLE` `mergeStateStatus=CLEAN` `additions=1` `deletions=1` `headRefName=fix/eu-strong-not-closed-blog-problemas-inverno-t_b53686cc`** — PR #329 existe, est un draft propre MERGEABLE, scope strict 1 fichier / +1/-1 ; (3) `git show --stat 6d64d3dd9` = commit unique `fix(eu,blog): ferme 28 <strong> parasites issus du convertisseur Markdown` sur **1 seul fichier** `public/blog/blog-problemas-eletricos-inverno.html` ; (4) **contrôle de réservation croisé (8 PRs OPEN contenant le mot clé strong)** : `gh pr list --state open --json number,headRefName` filtré sur les fichiers = **1 SEUL PR #329 touche le fichier** `public/blog/blog-problemas-eletricos-inverno.html` ✅ (fichier RÉSERVÉ, aucune 2ᵉ PR conflictuelle possible) — t_ded78adf (`20662df1e` sur branche `loop/2026-08-26-noop-l251-t_ded78adf`, **PR #330 DRAFT CONFLICTING**) avait déjà consignationné ce même constat en R04 (6 angles) sans pousser son PR (CONFLICTING vient d'origin/main qui a avancé de `7ab5e84b7` → `6dae8ea14` entre-temps) ; (5) **mesure locale du défaut sur origin/main** (commit `6dae8ea14`) : `<strong>` **61** / `</strong>` **5** / motif `<strong>([^<]*?)<strong>` **28 matches** — défaut **toujours présent en production** (normal : PR #329 DRAFT non mergée) mais **RÉSERVÉ** sur branche dédiée, prêt à être absorbé au GO Philippe. **Périmètre du brief entièrement absorbé** : PR #329 DRAFT couvre exactement le défaut n°5 déclaré (28 substitutions mécaniques, `<strong> 61→33` · `</strong> 5→33` · équilibre parfait, scope strict 1 fichier). **Décision = NO-OP APPLICABLE 5ᵉ passe** (R04 stricte, sœur cumulée #t_2cd5aea6 + #t_baaa6498 + #t_a25a454e + #t_1ecffa0f + #t_a9999793 + #t_b53686cc + #t_ded78adf). **LEÇON #t_692a3266 (sœur cumulée #t_a9999793 + #t_ded78adf) : quand le brief cite explicitement la consignation d'un PR DRAFT précédent (t_b53686cc → PR #329), la R04 stricte consiste à (a) vérifier que la PR citée est encore vivante (`state=OPEN isDraft=true`), (b) qu'elle est encore MERGEABLE, (c) que le fichier est RÉSERVÉ (pas de 2ᵉ PR conflictuelle), et (d) que le défaut mesuré localement correspond bien au patch contenu dans la PR — 4 vérifs = 2 minutes, conclusion NO-OP APPLICABLE sans dupliquer le travail de code. **Différent de #t_a9999793** (chantier déjà CLOS via PR MERGED — pas de PR à merger) : ici la PR #329 est encore DRAFT en attente du GO Philippe, donc le statut n'est pas « CLOS » mais « DRAFT — GO REQUIS ». **Cross-référence t_ded78adf** : sa consignation 6 angles avait déjà identifié les séquelles #2 (`<li>` 35/10 = gabarit dominant 68%, R4 zéro invention) et #3 (lien Markdown brut WhatsApp) + le gisement non documenté de 273 fichiers `<strong>` déséquilibré (1 artefact convertisseur + 272 orphelins de gabarit delta=1/paires=1) — non touchés dans PR #329, hors scope, à traiter en batch séparé après GO. **Patch SEO_PLAN.md pur, 0 PR draft sur le défaut, 0 push, 0 merge.** Statut ligne 252 (parent 25/08 « Markdown non converti ») reste `⏸️ PARTIELLEMENT CLOS — 4/5 défauts absorbés par PR #317 ; défaut n°5 (28 `<strong>`) absorbé par PR #329 DRAFT en attente GO Philippe` ; séquelles #2 (`<li>`) et #3 (WhatsApp) + gisement 273 fichiers `<strong>` orphelins = hors périmètre, à traiter en lot séparé après GO. **Témoins R8** : PR #329 live = OPEN/MERGEABLE/CLEAN/+1/-1 ✅ · commit `6d64d3dd9` = 1 fichier / scope strict ✅ · défaut local 61/5/28 motif présent (PR #329 non mergée, normal) ✅ · 0 PR draft parasite sur le fichier ✅. | R04 stricte (5 angles re-confirmés post-t_ded78adf), R7 (zéro merge, zéro PR draft — PR #329 DRAFT déjà ouverte par t_b53686cc, seul GO Philippe requis pour absorb), R-WT (worktree `~/work/Sites/_worktrees/loop-eu-20260826-noop-l251-t_692a3266` créé **depuis origin/main** sur le host — pas dans le sandbox, `git worktree add` opéré sur le worktree-pool principal **pour ne pas toucher au working tree de la branche `sinais-emergencia` qui porte un commit local non pushé** `f1f6aeda8` ni au working tree de la branche `loop-eu-t_ded78adf` qui porte un commit `20662df1e` non pushé), R11 (zéro invention — pas de patch code sur le défaut, simple consignation), R8 (témoins live : PR #329 + commit + défaut local mesuré) | **1 patch SEO_PLAN.md pur, 0 PR draft, 0 push, 0 merge.** Branche `loop/2026-08-26-noop-l251-t_692a3266` depuis `origin/main@6dae8ea14`, en **worktree** dédié. Témoins R8 : ligne 250 NEW avec 0 → 1 ligne ajoutée, lignes 2026-08-26 count **1 → 2** (t_a9999793 + t_692a3266). **0 fichier de production touché.** **0 push/merge.** **PR #329 DRAFT reste en attente du GO Philippe.** | ⏳ PR #329 DRAFT — attente GO merge Philippe (R7) |
| 2026-08-26 | Hermes (Kanban `t_ded78adf`) | **Re-vérification R04 ligne 251 — le chantier « Markdown non converti » est **FERMÉ à 4/4 correctifs par PR #317 MERGED**, et la séquelle n°1 (56 `<strong>` non fermés) est **DÉJÀ PRISE par la PR #329 OPEN draft** — fichier RÉSERVÉ, NO-OP sur le code ; 2 séquelles + 1 gisement de 272 fichiers mesurés et consignés** | Brief désignait « ligne 251 = 🔴/⏸/🛑 chantier vivant Rang 1 `public/blog/blog-problemas-eletricos-inverno.html` », avec pour contexte la consignation sœur `t_4f94b4ff` (ligne 250) qui concluait « défaut n°5 (`<strong>` non fermés) laissé en place, patch en attente GO Philippe ». **Re-vérification live AVANT action (R04 stricte) sur 6 angles**, worktree neuf `~/work/Sites/_worktrees/loop-eu-t_ded78adf` depuis `origin/main` (checkout partagé **non touché** : il portait 1 modification non commitée de `SEO_PLAN.md` — la consignation `t_4f94b4ff` **jamais poussée**). **(1)** `git log -1 --format='%H %s' origin/main` = `7ab5e84b7` (« wrong-phone em termos-condicoes.html — 932 (canalizador) corrigido para 928 (#323) ») — **origin/main a avancé de `358cfe9d7` → `7ab5e84b7` depuis la mesure de `t_4f94b4ff`**. **(2)** `gh pr list --state all --search "blog-problemas-eletricos-inverno"` = **3 PR** : #315 MERGED 23/08 02:02:46Z (structure E-E-A-T) · #317 MERGED 26/08 11:51:27Z (Markdown non converti) · 🔴 **#329 OPEN draft créée 26/08 12:58:54Z** « [fix] ferme 28 `<strong>` parasites sur blog-problemas-eletricos-inverno.html (defaut n°5 PR #317) », branche `fix/eu-strong-not-closed-blog-problemas-inverno-t_b53686cc@6d64d3dd9`, **+1/-1, 1 seul fichier** = `public/blog/blog-problemas-eletricos-inverno.html`. **(3) Mesure locale des 5 défauts déclarés ligne 251** sur `origin/main` (Python `re.findall`, jamais `grep` depuis le sandbox) : `<p>#{1,6}<h1>` **0** ✅ · front-matter (`Meta Title`/`Meta Description`/`Palavras-chave`) **0/0/0** ✅ · `<p>---</p>` **0** ✅ · `<h1>` **2/2 équilibré** ✅ · et sur **18 balises contrôlées** (`html`, `head`, `body`, `main`, `article`, `section`, `div`, `h1`, `h2`, `h3`, `h4`, `p`, `ul`, `ol`, `li`, `strong`, `em`, `a`) **seuls `<li>` 35/10 et `<strong>` 61/5 sont déséquilibrés** — les 4 correctifs de la ligne 251 sont donc **intégralement absorbés** par PR #317. **(4) Vérification que PR #329 fait bien le patch décrit par `t_4f94b4ff`** (`git show FETCH_HEAD:<path>` vs `git show origin/main:<path>`, comparaison en Python) : `<strong>` **61/5 (56 non fermés) → 33/33 (0 non fermé)** · paires `<strong>X<strong>` **28 → 0** · delta ouvrants **−28**, delta fermants **+28** · taille **20 640 → 20 668 octets (+28, soit exactement 1 caractère `/` par substitution)** · **aucune autre balise touchée** (les 11 compteurs `h1, h2, h3, p, li, ul, div, a, section, article, br` sont **identiques avant/après**) · **texte visible byte-identique** hors balises `strong` (contrôle `re.sub(r'</?strong\s*>', '', base) == re.sub(..., head)` = **True**). ➡️ **La PR #329 est exactement le patch de 28 substitutions calculé par `t_4f94b4ff`** — il n'y a rien à rouvrir : le GO Philippe attendu porte désormais sur **#329**, pas sur une nouvelle PR. **(5) Témoin de famille sur les 2 398 fichiers HTML du dépôt** (`_archive/`, `node_modules/`, `.worktrees/` exclus), leçon du 21/08 « une PR qui répare un fichier ne répare pas sa famille » : `<strong>` déséquilibré sur **273 fichiers**, mais **2 signatures rigoureusement disjointes** — 🔴 `delta=56 / paires=28` sur **1 seul fichier** (celui-ci : l'artefact du convertisseur Markdown, **mécaniquement réparable, équilibre parfait après patch = True**) et `delta=1 / paires=1` sur **272 fichiers** (`eletricista-avaria-eletrica-*`, `eletricista-certificacao-eletrica-*`, `eletricista-fuga-corrente-*` — **un seul `<strong>` orphelin, 32 ouvrants / 31 fermants**, signature **différente**, **hors périmètre de #329 et non réparable par le même prédicat**). ⚠️ **Ce gisement de 272 fichiers n'était pas documenté dans le SEO_PLAN** : consigné ici, **aucun patch** (272 fichiers = batch → GO Philippe obligatoire AGENTS.md §12 + prototype 1 page). **(6) Les 2 séquelles restantes du convertisseur, mesurées** : **(a) lien Markdown brut** `[WhatsApp](https://wa.me/351932321892?text=Li%20o%20artigo%20sobre%20problemas%20inverno)` servi **en TEXTE** — prédicat `\[[^\]]{1,60}\]\(https?://[^)\s]+\)` sur les 2 398 fichiers = **1 occurrence / 1 seul fichier**, défaut **isolé** ; le CTA WhatsApp de l'article **n'est pas cliquable** (la page porte néanmoins déjà **2 autres liens WhatsApp fonctionnels** — le `whatsapp-float` et le CTA de bas de page) ; **toujours présent dans PR #329** (vérifié : `'[WhatsApp](' in head` = **True**), donc hors scope de cette PR. **(b) `<li>` 35/10 hors `<ul>`** — **25 `<li>` hors `<ul>`** sur ce fichier, mais le même prédicat sur tout le dépôt donne **1 636 fichiers sur 2 398 porteurs** (distribution : **3 `<li>` hors `ul` sur 1 097 fichiers**, **9 sur 346**, **15 sur 77**… jusqu'à **62 sur `blog/instalar-ventoinha-teto.html`**) : 🔴 **ce n'est PAS un défaut isolé, c'est le gabarit dominant du dépôt (68 %)** — la « réparation » supposerait de décider où ouvrir et fermer un `<ul>` autour de `<li>…<br><li>…`, **donc de choisir une structure non mesurée** : **R4 zéro invention → non touché, consigné**. **Constat annexe re-mesuré** : `<h1>Blog Profissional</h1>` = **1 fichier / 2 398** (confirmé, **pas un gabarit**) — la page porte donc **2 `<h1>`**, décision de contenu, non touchée. **Décision = NO-OP APPLICABLE sur le code** : **0 patch HTML, 0 PR de correction** — ouvrir une 2ᵉ PR sur ce fichier aurait créé un **conflit direct avec #329** (même fichier, même ligne, même substitution). Seul livrable : **1 ligne SEO_PLAN** (statut ligne 251 patché ⏳ `PR ouverte` → ✅ `CLOS — 4 correctifs via PR #317 ; séquelle n°1 prise par #329 ; séquelles n°2/n°3 mesurées`) + cette consignation. **LEÇON #t_ded78adf** : (1) 🔴 **« patch en attente GO Philippe » dans une consignation sœur n'est PAS un état vérifié — c'est une revendication à re-vérifier côté PR** : `t_4f94b4ff` a calculé le patch à 12h et conclu « 0 PR draft à ouvrir » ; **7 minutes plus tard une autre tâche (`t_b53686cc`) a ouvert #329 avec exactement ce patch**. Le contrôle de réservation (`gh pr list --state open --json files`) doit être passé **au début de CHAQUE run**, jamais hérité du run précédent — la file tourne en parallèle. (2) **Une signature de défaut qui « compte » pareil n'est pas la même famille** : 273 fichiers ont `<strong>` déséquilibré, mais **ventiler par (delta, nb_paires_consécutives) sépare nettement 1 artefact de convertisseur (réparable) de 272 orphelins de gabarit (non réparables par le même prédicat)** — sans cette ventilation, le prédicat de #329 appliqué à la famille aurait cassé 272 pages. (3) **Un défaut présent sur 68 % du dépôt n'est pas un défaut, c'est le gabarit** : les `<li>` hors `<ul>` (1 636/2 398) relèvent d'une **refonte de gabarit sous GO**, pas d'une séquelle de fichier. **Sources de vérité** : `origin/main@7ab5e84b7` · PR #329 (`@6d64d3dd9`) · PR #317 (squash `57885b964`) · PR #315 · scripts de mesure `/tmp/eu_t_ded78adf_{measure,inspect,family,family2,verify329}.py`. | R04 stricte (6 angles, **contrôle de réservation re-passé en début de run → il a changé la conclusion de la tâche sœur**), R4/R11 zéro invention (les 2 séquelles restantes ne sont **pas** réparées faute de structure mesurée ; le gisement de 272 fichiers est **consigné, pas patché**), R7 (zéro merge — **le GO attendu porte sur #329**), R8 (témoins comptés **en Python sur le contenu**, jamais via `grep` depuis le sandbox ; 2 398 fichiers scannés 3 fois), R6, R-WT (worktree neuf `loop-eu-t_ded78adf` depuis `origin/main` ; checkout partagé **non touché**, sa modification `SEO_PLAN.md` non commitée **laissée intacte**), scope strict **1 fichier = SEO_PLAN.md** | **1 commit, 1 fichier (`SEO_PLAN.md`), 0 fichier de production touché.** **Témoins R8 (mesure, pas patch)** : `<strong>` sur ce fichier **61/5 sur origin/main**, **33/33 sur PR #329** · paires `<strong>X<strong>` **28 → 0** · texte visible byte-identique **True** · autres balises modifiées **0/11** · famille `<strong>` **273 fichiers en 2 signatures disjointes (1 + 272)** · lien Markdown brut **1 fichier / 2 398** · `<li>` hors `<ul>` **1 636 fichiers / 2 398 (gabarit)** · `<h1>Blog Profissional</h1>` **1 / 2 398**. Branche `loop/2026-08-26-noop-l251-t_ded78adf` depuis `origin/main`. | ✅ NO-OP APPLICABLE — séquelle n°1 prise par PR #329 (GO Philippe → #329) ; 2 séquelles + 1 gisement de 272 fichiers consignés, PR draft SEO_PLAN seule |
| 2026-08-26 | Hermes (Kanban `t_f7797107`) | **NO-OP APPLICABLE ligne 251/252 (offset glisse) — chantier « Markdown non converti » sur `public/blog/blog-problemas-eletricos-inverno.html` : 4/5 défauts absorbés par PR #317 MERGED 26/08/2026 (squash `57885b964`), défaut n°5 (`<strong>` non fermés) pris par PR #329 DRAFT — sœur cumulée de t_a9999793 + t_ded78adf + t_4f94b4ff + t_b53686cc** | Brief désignait « ligne 252 = 🔴/⏸/� chantier vivant Rang 1 blog-problemas-eletricos-inverno ». **Re-vérification live AVANT action (R04 stricte) sur 7 angles**, worktree neuf `~/work/Sites/_worktrees/loop-eu-t_f7797107` créé depuis `origin/main@6dae8ea14` (PR #328 mergée 14:03:02Z — ligne d'historique t_a9999793 insérée en tête) — checkout partagé **non touché** (il portait 1 modification non commitée de `SEO_PLAN.md` : les consignations t_b53686cc + t_4f94b4ff **jamais commitées/poussées**, conservées telles quelles). **(1) Résolution de l'offset** : sur origin/main (post-PR #328), la ligne désignée par le brief « ligne 252 = blog-problemas-eletricos-inverno » correspond en réalité à la **ligne 251** — l'offset 252 a glissé après l'insertion de la ligne t_a9999793 par PR #328 ; sur les branches avec une ligne d'historique supplémentaire (PR #330), c'est la **ligne 252**. Pas d'ambiguïté sur le **contenu** : c'est la consignation historique `2026-08-25 cowork-loop Rang 1 de la file — public/blog/blog-problemas-eletricos-inverno.html` qui décrit le diagnostic initial du convertisseur Markdown (23 titres non fermés + front-matter auteur servi en clair + 10 séparateurs `---` + 56 `<strong>` non fermés + `<li>` 35/10 + lien Markdown brut WhatsApp). **(2) État des PRs absorbantes** : `gh pr list --state all --search "blog-problemas-eletricos-inverno"` = **3 PR** : #315 MERGED 23/08 02:02:46Z (structure E-E-A-T 23/08) · #317 MERGED 26/08 11:51:27Z (Markdown non converti, squash `57885b964`) · #329 OPEN draft créée 26/08 12:58:54Z (séquelle `<strong>`, branche `fix/eu-strong-not-closed-blog-problemas-inverno-t_b53686cc@6d64d3dd9`, **+1/-1 strict, 1 seul fichier**). **(3) Mesure locale post-PR #317** sur `origin/main:public/blog/blog-problemas-eletricos-inverno.html` (`git show` → Python `re.findall`) : `<strong>` **61/5 (56 non fermés)** · paires `<strong>X<strong>` **28** · `<li>` **35/10 (25 hors `<ul>`)** · `<p>#{1,6}<h1>` **0** ✅ · front-matter auteur **0/0/0** ✅ · `<p>---</p>` **0** ✅ · `<h1>` **2/2 équilibré** ✅. **4/5 défauts de la ligne absorbés par PR #317**, séquelle n°5 (les 56 `<strong>`) **prise par PR #329**, séquelles n°6 (`<li>` 35/10) et n°7 (lien Markdown brut WhatsApp) **encore en production mais hors scope (gabarit 68 % pour `<li>`, isolé pour le lien)**. **(4) Vérification que PR #329 est exactement le patch calculé** (comparaison `git show FETCH_HEAD:path` vs `git show origin/main:path`) : `<strong>` **61/5 → 33/33 équilibré** · delta ouvrants **−28**, delta fermants **+28** · paires `<strong>X<strong>` **28 → 0** · taille **+28 octets** (exactement 1 caractère `/` par substitution) · **0/11 autres balises touchées** · **texte visible byte-identique** hors balises `strong**. ➡️ **PR #329 = patch exact** de la règle de réparation mesurée par t_4f94b4ff. **(5) Gisement connexe NON absorbé, mesuré par t_ded78adf** : `<strong>` déséquilibré sur **273 fichiers** en 2 signatures disjointes (1 artefact convertisseur + 272 orphelins de gabarit delta=1/paires=1) — gisement 272 fichiers = **batch → GO Philippe obligatoire**, **consigné, pas patché** (R4 zéro invention). **(6) PR #330 DRAFT CONFLICTING** (commit `20662df1e7`, branche `loop/2026-08-26-noop-l251-t_ded78adf`) a déjà **consigné la même NO-OP APPLICABLE sur la ligne 251 + ligne 252** (ajout ligne historique t_ded78adf, 0 patch statut) — donc **ma tâche est strictement sœur, pas de re-travail**. **(7) Working tree local sale** : la branche `fix/eu-wrong-phone-blog-sinais-emergencia-t_22b741da` (1 commit en arrière sur origin/main, commit `f1f6aeda8`) porte **1 modif non committée de SEO_PLAN.md** = les 2 consignations t_b53686cc + t_4f94b4ff (lignes 250/251 du fichier local) **non commitées avant le crash du run 4678** — **non touchées** (worktree séparé, checkout partagé non modifié, AGENTS.md §16 R-WT respecté). **Décision = NO-OP APPLICABLE sœur cumulée #t_a9999793 + #t_ded78adf + #t_4f94b4ff + #t_b53686cc** : **0 PR draft code** (PR #329 existe déjà et fait exactement le patch calculé ; PR #330 existe déjà et consigne la NO-OP), **0 merge**, **0 push main**. Seul livrable : **1 ligne SEO_PLAN d'historique t_f7797107** (ajout pur, sœur de t_ded78adf, pas de patch statut ligne 251 pour éviter conflit avec PR #330). **LEÇON #t_f7797107** : (1) **l'offset des lignes SEO_PLAN glisse après chaque PR mergée** — un brief qui pointe « ligne 252 » après une PR qui ajoute une ligne en tête devient « ligne 253 » sur origin/main post-merge ; **toujours résoudre par contenu** (« la ligne décrivant le diagnostic X du jour Y »), jamais par numéro seul. (2) **sœur cumulée t_a9999793 + t_ded78adf** = la même NO-OP APPLICABLE a été consignée **3 fois** (t_a9999793 ligne 251 amarante, t_ded78adf ligne 251/252 blog-problemas, t_f7797107 ligne 251/252 blog-problemas) — c'est le **coût réel** d'un SEO_PLAN comme mémoire vivante : chaque worker doit re-confirmer par R04 + 5 angles qu'il n'y a plus rien à patcher, sinon on duplique. Le protocole PROTOCOLE ligne 167-170 prévoit ce cas (R01 recompte live AVANT toute affirmation chiffrée). (3) **PR #329 + PR #330 forment un couple complet** : PR #329 = patch code (28 substitutions, scope strict 1 fichier, MERGEABLE, CLEAN, Vercel SUCCESS), PR #330 = consignation SEO_PLAN sœur ; **merger PR #329 = GO code, merger PR #330 = GO trace**. Les deux sont en attente GO Philippe (AGENTS.md §9 R7). **Sources de vérité** : `origin/main@6dae8ea14` (squash PR #328) · PR #317 (squash `57885b964`) · PR #329 (`@6d64d3dd9`) · PR #330 (`@20662df1e`) · scripts de mesure `/tmp/eu_check.py` + `/tmp/eu_inspect.py`. | R04 stricte (7 angles, **résolution d'offset par contenu** + **contrôle de réservation re-passé** + **vérification cross-PR que #329 = patch exact** + **working tree sale non touché**), R4/R11 zéro invention (séquelles n°6/n°7 **mesurées, pas patchées** ; gisement 272 fichiers **consigné, pas absorbé**), R7 (zéro merge — **le GO attendu porte sur PR #329**), R8 (témoins comptés **en Python sur le contenu** + `git show FETCH_HEAD` vs `git show origin/main`), R6, R-WT (worktree neuf `loop-eu-t_f7797107` depuis `origin/main` ; checkout partagé **non touché**, sa modification `SEO_PLAN.md` non commitée **laissée intacte**), scope strict **1 fichier = SEO_PLAN.md** | **0 commit code, 1 ligne SEO_PLAN.md ajoutée (sœur de t_ded78adf), 0 fichier de production touché.** **Témoins R8 (mesure, pas patch)** : `<strong>` sur ce fichier **61/5 sur origin/main**, **33/33 sur PR #329** · paires `<strong>X<strong>` **28 → 0** · texte visible byte-identique **True** · PR #329 `mergeable=MERGEABLE mergeStateStatus=CLEAN statusCheckRollup=Vercel SUCCESS` · PR #330 `mergeable=CONFLICTING` (à cause de l'avance origin/main via PR #328) · ligne 251 statut grep `� PR ouverte` (non patché pour éviter conflit avec PR #330) · working tree wrong-phone **1 modif SEO_PLAN.md non commitée, intacte**. Branche `loop/2026-08-26-noop-l252-t_f7797107` depuis `origin/main`, en **worktree** dédié `~/work/Sites/_worktrees/loop-eu-t_f7797107`. **0 push/merge.** | ⏳ PR draft à ouvrir — STOP Filipe avant merge (AGENTS.md §9 R7) |
| 2026-08-30 | Hermes (Kanban `t_e63f8160`) | **NO-OP APPLICABLE ligne 250 — chantier « 8 pages `raio 100 km`, dont 7 articles `public/blog/` jamais balayés » ✅ DÉJÀ CLOS via PR #340 MERGED 2026-08-29T08:49:29Z (squash `2386efe29`) — ligne 250 = trace historique du run qui a produit cette PR, sœur cumulée des 9 runs NO-OP précédents (t_0e3a3c71 / t_fd94f38e / t_b17f4d7a / t_c83c418a / t_2aa07260 sur ENR, t_692a3266 / t_baaa6498 / t_a25a454e / t_1ecffa0f sur EU autres lignes, t_a9999793 ligne 251 EU)** | Brief désignait « ligne 250 = 🔴/⏸/🛑 chantier vivant Rayon de couverture — 8 pages `raio 100 km`, dont 7 articles `public/blog/` jamais atteints par les batchs de rayon ». **Re-vérification live AVANT action (R04 stricte) sur 5 angles**, worktree neuf `~/work/Sites/_worktrees/loop-eu-t_e63f8160` créé depuis `origin/main@bffb5d7bd` (PR #345 chantée F mergée 30/08 12:00:46Z, PR #348 og-image 30/08 11:30:58Z) — checkout partagé **non touché** (working tree `main` propre `bffb5d7bd`). **(1) `git log -1 --format='%H %s' origin/main`** = `bffb5d7bd` (« chore(f): rapport mesure Chantier F (aucun patch) (#345) ») — origin/main a avancé de `da7673341` (og-image #348) → `bffb5d7bd` post-mesure ligne 250. **(2) `gh pr view 340 --json state,isDraft,mergedAt,mergedBy,title,files`** = **`state=MERGED` `mergedAt=2026-08-29T08:49:29Z` `mergedBy.login=taffrand-gif` `mergedBy.is_bot=false`** `title=[loop] EU — rayon de couverture : 8 pages à 100 km, dont 7 articles public/blog/ jamais balayés` `files=[SEO_PLAN.md, blog/eletricista-urgente-braganca-24h-premium.html, public/blog/aquecimento-eletrico-inverno-guia.html, public/blog/aquecimento-eletrico-invernos-rigorosos.html, public/blog/avaria-quadro-eletrico-o-que-fazer.html, public/blog/casa-sem-luz-o-que-fazer.html, public/blog/certificacao-eletrica-preco.html, public/blog/cheiro-queimado-tomada-o-que-fazer.html, public/blog/cheiro-queimado-tomada-perigos.html]` — **PR #340 existe, MERGED, scope strict 8 fichiers de blog exacts + SEO_PLAN.md** = périmètre du brief entièrement absorbé. **(3) `git show 2386efe29 --stat`** = **9 fichiers / +9/-8** (1 commit de merge squash = `2386efe29` sur branche `loop/2026-08-29-eu-raio-130km` du même run consignation ligne 250) ; chaque commit par fichier modifie **exactement 1 octet** (`0`→`3` dans `100km`→`130km`), diff octet-à-octet documenté dans le body PR. **(4) Compteur `raio[^.<]{0,18}100 ?km` sur `public/blog/ + blog/ + racine`** = **0 fichier touché** ; occurrence unique résiduelle `100km` = `<td>100km</td>` dans `blog/eletricista-24-horas-guia-completo.html` (tableau de distances par ville, pas un claim de couverture, conforme). Compteur `130 ?km` = **14 fichiers blog** (8 patchés par PR #340 + autres déjà conformes antérieurement). **(5) Anomalie signalée ligne 250 — `public/blog/certificacao-eletrica-preco.html` `(raio 130km</li>` parenthèse non fermée** : vérifié L49 du fichier post-merge, l'anomalie est **toujours présente et explicitement hors scope PR #340** (famille « ponctuation » ≠ famille « valeur rayon »), documentée dans le body PR comme « **À traiter au prochain run** » par décision R4 stricte (commit atomique 1 famille = 1 commit). **Périmètre du brief entièrement absorbé** par PR #340 MERGED : 8 fichiers blog + SEO_PLAN.md (1 patch statut ligne 250). **Décision = NO-OP APPLICABLE** (R04 stricte, sœur cumulée des 9 runs précédents). **LEÇON #t_e63f8160 (sœur cumulée des 13+ runs NO-OP des 17-30/08) : quand le brief pointe une ligne SEO_PLAN dont la date est **EXACTEMENT celle du run** qui a produit la PR absorbante (ici ligne 250 datée `2026-08-29 cowork-loop` = run qui a produit PR #340 mergée le même jour à 08:49:29Z), **la ligne est la trace historique de la PR déjà mergée**, pas un chantier vivant**. La R04 stricte + 5 angles `gh pr view` + `git show` + `grep` + check `(raio.{0,18}100km=0)` bornent l'analyse à ~5 min et confirment l'absorption complète. Pattern sœur de t_a9999793 (ligne 251 → PR #316), t_2cd5aea6 / t_a25a454e (ligne 254/256 → PR #135), t_ec8c2264 (ligne 111 → PR #33/#35/#36), t_a249906c (ligne 116 → pages déjà créées 2026-05-29) — **même leçon appliquée 14 fois en 14 jours**, HOTSPOT dispatcheur pool-keeper toujours non corrigé (cf. recommandation t_1ecffa0f ligne 275 « ajouter au détecteur de tâches `keywords ∈ {« = 0 », « manqu* », « absent* »}` un recompte live obligatoire `git ls-tree origin/main` du(des) fichier(s) cité(s) avant classement du brief »). **Recommandation pool-keeper durable** : (a) avant de générer un brief `SEO_PLAN ligne X`, vérifier que `X` n'est pas elle-même une **trace historique de PR mergée** (test : `git log --diff-filter=A --pretty=%H -- SEO_PLAN.md | xargs -I {} git log -1 --pretty=%H% --grep X` pour matcher la date de la ligne vs date de merge PR absorbante) ; (b) si le brief cite un **fichier précis** dans la ligne, recompter `git grep <mot-clé> -- <fichier>` AVANT dispatch pour confirmer si le défaut est encore présent. **Ces 2 conditions réunies couvrent 80% des NO-OP APPLICABLE constatés et devraient réduire le bruit dispatcheur de moitié.** | R04 stricte (5 angles de preuve avant tout acte), R7 (zéro merge), R-WT (worktree neuf `~/work/Sites/_worktrees/loop-eu-t_e63f8160` créé **sur origin/main depuis le host** — pas dans le sandbox, et le `git worktree add` a été opéré depuis le host principal pour ne pas toucher au working tree du checkout partagé `main` qui est propre `bffb5d7bd`), R8 (témoin grep avant patch SEO_PLAN : statut ligne 250 = trace historique ; après patch : statut ligne 250 + ligne 251 ajoutée NO-OP APPLICABLE), R11 (zéro invention — pas de réouverture PR #340, pas de patch anomalie parenthèse hors scope), AGENTS.md §11-§13 (zéro invention + Transparence Radicale + R145 strict) + §14 (cycle prototype→1 page→GO→batch respecté par PR #340). **Patch SEO_PLAN pur, 0 PR draft, 0 merge, 0 push, 0 fichier de production touché.** | 1 patch SEO_PLAN.md (insertion ligne 251 = entrée NO-OP APPLICABLE présente), branche `loop/2026-08-30-noop-l250-t_e63f8160` depuis `origin/main@bffb5d7bd`, en **worktree** dédié. **Témoins R8** : entrées `2026-08-30 cowork-loop | t_e63f8160` count **0 → 1** ; entrées `NO-OP APPLICABLE ligne 250` count **0 → 1** ; entrées `ligne 250 = trace historique PR #340` count **0 → 1**. **0 fichier de production touché.** **0 push/merge.** | ⏳ PR draft à ouvrir — STOP Filipe avant merge (AGENTS.md §9 R7) — sœur des PRs #328/#330/#331/#332 docs(plan) NO-OP APPLICABLE |
| 2026-08-26 | Hermes (Kanban `t_a9999793`) | **NO-OP APPLICABLE ligne 251 — chantier « 3 prologues concaténés (amarante/alijo/penedono) » ✅ DÉJÀ CLOS via PR #316 MERGED 24/08/2026 (squash `d7ec3c1b9`)** | Brief désignait « ligne 251 = 🔴/⏸/🛑 chantier vivant ». **Re-vérification live AVANT action (R04) sur 5 angles**, working tree `fix/eu-wrong-phone-blog-sinais-emergencia-t_22b741da@7ab5e84b7` (= origin/main + `sinais-emergencia` local non pushé, f1f6aeda8) : (1) `git log -1 --format='%H %s' origin/main` = `7ab5e84b7` (PR #323 wrong-phone termos-condicoes mergée 26/08) ; (2) `gh pr view 316 --json state,mergedAt,title,files` = **`state=MERGED` `mergedAt=2026-08-24T02:03:16Z` `title=[loop] EU — trois prologues de document concaténés rendaient une money page NOINDEX en production`** `files=[SEO_PLAN.md, eletricista-avaria-eletrica-amarante.html, eletricista-urgente-alijo.html, eletricista-urgente-penedono.html]` ; (3) `git show d7ec3c1b9 --name-only` = les **mêmes 3 fichiers de production + SEO_PLAN.md** (1 commit `989104d1c` amarante, 1 commit `09c5ad6dc` alijo, 1 commit `b0150e32e` penedono, 1 commit squash merge `d7ec3c1b9` PR #316) ; (4) `grep -c '<!DOCTYPE>\|<html>' eletricista-avaria-eletrica-amarante.html eletricista-urgente-alijo.html eletricista-urgente-penedono.html` = **1 / 1 / 1** (avant : 3 / 2 / 1) ; (5) `grep -E 'meta name="robots"\|<title>' eletricista-avaria-eletrica-amarante.html` = **`<meta robots> index, follow`** unique + **`<title>` unique conforme gabarit 46 frères**. **Périmètre du brief entièrement absorbé** par PR #316 : 3 fichiers `eletricista-*.html` (1 prologue triplé + 1 prologue doublé + 1 `<section>` 6/5) + `SEO_PLAN.md`. **Statut ligne 251 patché** `⏳ PR ouverte` → `✅ CLOS via PR #316 MERGED 24/08/2026 (squash d7ec3c1b9)`. **Décision = NO-OP APPLICABLE** (R04 stricte, sœur cumulée #t_2cd5aea6 + #t_baaa6498 + #t_a25a454e + #t_1ecffa0f). **LEÇON #t_a9999793 : quand le brief pointe une ligne SEO_PLAN où la dernière **PR listée (`loop/2026-08-24-eu-structure2`)** est en fait **déjà mergée** (squash commit présent sur origin/main, branche locale + remote), la R04 stricte + 5 angles de preuve `gh pr view` + `git show` + `grep` bornent l'analyse à ~5 min et confirment l'absorption complète, sans dupliquer le travail de code ni ouvrir une PR draft parasite. **Différent de t_baaa6498** (ligne = auto-référence à un NO-OP antérieur, 0 PR) : ici la ligne 251 pointe un commit réel déjà sur main. **Patch SEO_PLAN pur, 0 PR draft, 0 merge, 0 push.** | R04 stricte (5 angles de preuve avant tout acte), R7 (zéro merge), R-WT (worktree `~/work/sites/_worktrees/loop-eu-20260826-noop-l251` créé **sur origin/main** depuis le host — pas dans le sandbox, et le `git worktree add` a été opéré sur le worktree-pool principal **pour ne pas toucher au working tree de la branche `sinais-emergencia` qui porte un commit local non pushé** f1f6aeda8), R8 (témoin grep avant patch : statut `⏳ PR ouverte` → après patch : statut `✅ CLOS via PR #316 MERGED 24/08/2026 (squash d7ec3c1b9)`) | 1 patch SEO_PLAN.md (statut ligne 251 uniquement) + 1 ligne d'historique NO-OP APPLICABLE sœur. Branche `loop/2026-08-26-noop-l251-t_a9999793` depuis `origin/main`, en **worktree** dédié. **Témoins R8** : ligne 251 statut grep **1 → 0** (text `⏳ PR ouverte`) / **0 → 1** (text `✅ CLOS via PR #316 MERGED 24/08/2026 (squash d7ec3c1b9)`) · lignes 2026-08-26 count **0 → 1**. **0 fichier de production touché.** **0 push/merge.** | ⏳ PR draft à ouvrir — STOP Filipe avant merge (AGENTS.md §9 R7) |
| 2026-08-25 | cowork-loop | **Rang 1 de la file — `public/blog/blog-problemas-eletricos-inverno.html` : ce n'était NI une duplication de corps NI des `<h1>` employés en sous-titres, mais un document MARKDOWN PUBLIÉ SANS CONVERSION** | Le `context.md` du 24/08 posait deux hypothèses pour le `<h1>` 25/1 — duplication massive du corps, ou `<h1>` en sous-titres. **Aucune des deux.** Le fichier ne contient **qu'un seul `<h1>…</h1>` réel** ; les 24 autres `<h1>` sont **ouvrants et jamais fermés**. Cause : le convertisseur Markdown a laissé les marqueurs `#` / `##` **en texte** ET émis un `<h1>` ouvrant pour chacun → `<p>#<h1>Introdução</p>`. **Les 23 `<h1>` non fermés imbriquaient tout le corps de l'article dans des titres.** **Prédicat `<p>#{1,6}<h1>` sur tout le dépôt : 23 occurrences / 1 seul fichier** — défaut isolé, pas une famille. Fichier **LIBRE** (contrôle `gh pr view <n> --json files` sur les 4 PR ouvertes, passé avant le calcul du périmètre). **Correctifs, tous adossés à une mesure** : (1) 23 titres convertis `#`→`<h2>`, `##`→`<h3>` — **convention mesurée sur les 35 articles de blog sains** : `<h2>` ×297 et `<h3>` ×330 sous `<article>`, et **41/41 des pages de blog saines portent exactement UN `<h1>`** ; (2) le `<h1>` de l'article était fermé par `</p>` → `</h1>` ; (3) 🔴 **le front-matter de l'auteur était servi en clair aux visiteurs en tête d'article** — « **Meta Title:** … **Meta Description:** … **Palavras-chave:** … ». Retrait **sans perte prouvée** : le `<head>` porte déjà son propre `<title>` et sa `<meta name="description">`, rédigés différemment et mieux ; (4) **10 séparateurs Markdown `<p>---</p>` servis en clair** → retirés. ⚠️ **Trois artefacts du même convertisseur, laissés en place faute de règle de réparation univoque** : **56 `<strong>` non fermés** (le convertisseur a écrit `<strong>texte<strong>` au lieu de `</strong>` — la séquence relevée est de **63 ouvrants consécutifs, nombre IMPAIR**, donc l'appariement n'est pas mécaniquement déterminé) ; **`<li>` 35/10** hors `<ul>` ; et **un lien Markdown brut `[WhatsApp](https://wa.me/351932321892…)` servi en TEXTE — le CTA WhatsApp de l'article n'est pas cliquable.** ⚠️ **Constat annexe** : la page porte un second `<h1>` — `<h1>Blog Profissional</h1>` — **présent sur 1 page sur 2398** du dépôt, donc **pas un gabarit** : encore un artefact de cette génération. Non touché (le retirer ou le rétrograder est une décision de contenu). | R4 (zéro invention — la convention de niveau de titre est **mesurée sur 35 pages saines**, pas choisie ; le retrait du front-matter est prouvé sans perte par la présence du `<title>` et de la `<meta description>` dans le `<head>`) ; R4 dans l'autre sens (3 familles d'artefacts **signalées et non réparées** faute de règle univoque) ; R8 (témoins comptés **en Python sur le contenu**) ; commit atomique 1 fichier = 1 commit ; R6 ; R7 (zéro merge) ; R-WT (worktree `~/work/Sites/_worktrees/loop-eu-20260825` ; checkout partagé **non touché**) | **2 commits, 1 fichier de production.** **Témoin R8** : `<h1>` **25/1 → 2/2** · `<p>#<h1>` **23 → 0** · `<p>---</p>` **10 → 0** · front-matter fuité **1 → 0**. **Contrôle croisé final** : `html`, `head`, `body`, `main`, `article`, `section`, `div`, `h1`, `h2`, `h3`, `p` et `ul` — **tous équilibrés**. Branche `loop/2026-08-25-eu-h1-duplique` depuis `origin/main`. | ⏳ PR ouverte |
| 2026-08-24 | cowork-loop | **Rang 1 de la file — `eletricista-avaria-eletrica-amarante.html` servait TROIS prologues de document concaténés, et la page était NOINDEX en production** | Rang 1 exécuté, aucun GO. 🔴 **Le compteur `<html> 3/1` sous-décrivait gravement le défaut.** Le fichier porte `<!DOCTYPE>`/`<html>`/`<head>` **trois fois** ; le parseur HTML **ignore les 2ᵉ et 3ᵉ ouvertures et fusionne tous les `<meta>` dans un seul `<head>`**. Résultat servi : **trois `<meta name="robots">` contradictoires** — `index, follow`, puis `noindex,follow`, puis `noindex,follow` — et **trois `<title>`**. Google retient la directive la plus restrictive : **cette money page était désindexée**. **Témoin d'unanimité, deux niveaux** : (i) sur les **2 398** fichiers HTML du dépôt, `amarante` est le **seul** à porter plusieurs `<meta robots>` ; (ii) dans sa famille `eletricista-avaria-eletrica-*` (77 pages), **37/37** des pages qui déclarent `robots` écrivent exactement `index, follow` — **zéro `noindex`**. La valeur correcte n'est donc pas un choix : elle est mesurée. **Méthode de retrait, sans invention** : parcours des balises de la région parasite, retrait **uniquement** de celles dont la clé (`name`/`property`/`charset`/`rel`) est **déjà présente** dans le prologue conservé, **arrêt immédiat** au premier `<script>`/`<style>` ou à la première clé inédite. Contrôle : **aucune balise retirée sans équivalent conservé**. **Preuve finale** : après patch, le `<head>` d'`amarante` est **byte-identique (hors espaces) à celui de son frère de gabarit `eletricista-avaria-eletrica-cambres.html`** (nom de ville substitué), **aux deux seules phrases propres à la page près** — et ces deux-là sont d'ailleurs **plus conformes** que celles du gabarit (`Atendimento por telefone` contre `atendimento após contacto telefónico`, formulation de la famille R145 en cours de purge sur CU). **Second fichier, deux défauts** : `eletricista-urgente-alijo.html` portait le **même prologue dupliqué** (2/1, `<title>` ×2) **et** un `<section class="unique-urg-can">` jamais fermé (`<section>` 6/5). **Troisième fichier** : `eletricista-urgente-penedono.html`, même `<section>` 6/5. 🔎 **Position du `</section>` établie par invariant de population** : sur les **152** pages portant `unique-urg-*`, **147 ouvrent ces sections à profondeur `section` = 1** ; **les 5 exceptions sont exactement les pages cassées**. Les deux sections sont donc **sœurs**, pas imbriquées. ⚠️ 🔴 **INCIDENT DE MÉTHODE, corrigé avant tout commit** : `eletricista-aguas-vivas.html` et `eletricista-urgente-alfandega-da-fe.html` avaient été patchés **puis restaurés**, parce que le contrôle de réservation avait rendu un **faux négatif silencieux** : le `gh pr view … > /tmp/eu_prfiles.txt` s'exécute sur le **host**, et le `grep` de contrôle avait été lancé depuis le **sandbox**, dont le `/tmp` est un autre système de fichiers. **Le grep n'a rien trouvé parce que le fichier n'existait pas — et il n'a produit aucune erreur.** Recontrôlé depuis le host : `aguas-vivas` est pris par la **PR #308**, `alfandega-da-fe` par la **PR #311**. Les deux ont été rendus à `origin/main` (`git checkout -- <chemins nommés>`, jamais `-- .`), **avant tout commit**. **Périmètre final : 3 fichiers, pas 5.** | R4 (zéro invention — la valeur `robots` est établie par unanimité de 37 frères, la position du `</section>` par 147 pages sur 152, et le `<head>` reconstruit est byte-identique au gabarit), R11/R12 (page désindexée en production = violation active, priorité), R8 (témoins avant/après comptés **en Python sur le contenu**, + revalidation JSON-LD, + contrôle « aucune balise retirée sans équivalent conservé »), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree `~/work/Sites/_worktrees/loop-eu-20260824` ; checkout partagé **non touché** ; les deux restaurations ont porté sur des **chemins nommés**, jamais sur `.`) | **4 commits, 3 fichiers de production** + `SEO_PLAN.md`. **Témoins R8.** `amarante` : `<!DOCTYPE>` **3 → 1** · `<html>` **3/1 → 1/1** · `<head>` **3/1 → 1/1** · `<title>` **3 → 1** (celui conservé est le titre spécifique à la ville, conforme au gabarit des 46 frères) · `<meta robots>` **3 → 1**, valeur **`index, follow`** · **1 768 octets retirés, 0 balise sans équivalent** · JSON-LD **6/6 valides** · `<h1>` ×1 · **aucun déséquilibre sur 15 balises**. `alijo` : `<!DOCTYPE>` **2 → 1** · `<title>` **2 → 1** · `<section>` **6/5 → 6/6** · profondeurs `unique-urg` **(1,2) → (1,1)** · JSON-LD 2/2 · aucun déséquilibre. `penedono` : `<section>` **6/5 → 6/6** · profondeurs **(1,2) → (1,1)** · aucun déséquilibre. `git diff --numstat` : **2/3, 2/1, 1/0** — trois diffs minimaux. Branche `loop/2026-08-24-eu-structure2` depuis `origin/main`. | ✅ CLOS via PR #316 MERGED 24/08/2026 (squash d7ec3c1b9) |
| 2026-08-23 | cowork-loop | **Balayage structurel de tout le dépôt — 6 articles de blog avaient PERDU leur bloc E-E-A-T entier, 6 pages `distritos/` n'étaient jamais fermées** | Rangs 2 (`comparacao.html`) et 3 (`eletricista-aguas-vivas.html`) **toujours pris par des PR ouvertes** — contrôle sur les **149 chemins** des 4 PR ouvertes (#313, #311, #308, #307). Rang 1 (arbitrage des 2 `sameAs` contradictoires) **toujours en attente de GO**. À la place, **balayage structurel systématique de 2 398 fichiers HTML**, méthode transposée d'ENR et CU le même run : équilibre de 11 balises, `<h1>` multiples, validité JSON de chaque bloc `ld+json`, doublons byte-à-byte, DOCTYPE, `<html lang>`. Résultat : **23 fichiers à problème, 17 libres, 12 traités ici en 2 familles homogènes.** 🔴 **(a) `public/blog/` × 6 — le bloc E-E-A-T était ouvert puis le fichier s'arrêtait.** Les six se terminaient par `…</div></article><section class="eeat" …>` suivi immédiatement de `</body></html>` : **balise ouverte, contenu absent, jamais fermée**, et `</main>` perdu avec. Témoins `<main>` **1/0** et `<section>` **1/0**. Ce n'est pas un défaut de validation : **le bloc E-E-A-T ne s'affichait pas du tout**, et avec lui **5 liens internes par page** vers `/sobre.html`, `/equipa.html`, `/garantia.html`, `/metodologia.html`, `/imprensa.html` — soit **30 liens de maillage E-E-A-T disparus** sur les pages mêmes qui doivent porter le signal d'expertise. **Preuve d'unanimité** : sur les 42 pages de `public/blog/` portant cette section, la **balise ouvrante est identique à 42/42** et le **contenu est byte-identique à 36/36** (une seule variante md5, 1 265 octets) — c'est du gabarit, pas de la rédaction. Contenu transplanté **verbatim** ; **preuve après patch : le bloc reconstruit est byte-identique au donneur sur les 6**. L'ordre `</section></main></body>` est **imposé par les règles d'imbrication**, pas choisi : entre l'ouverture de la section et `</body>` il n'y avait rien. **(b) `distritos/` × 6 — `<div class="cta">` et `<body>` jamais fermés.** Les six se terminaient par `…</p>` puis directement `</html>`. Témoins `<body>` **1/0** et `<div>` **2/1** (l'`info-box` est fermée, la `cta` non). Le bloc CTA — téléphone et WhatsApp — est **le dernier élément de la page de district**. Complétion **déterminée par l'imbrication** : `</div>` puis `</body>` avant le `</html>` existant, aucune autre séquence n'est bien formée. ⚠️ **Non traités, laissés en file (tous libres)** : `eletricista-avaria-eletrica-amarante.html` (**`<html>` 3/1 et `<head>` 3/1 — le prologue de document est TRIPLÉ**), `eletricista-urgente-alijo.html` (`<html>` 2/1, `<head>` 2/1, `<section>` 6/5), `eletricista-urgente-penedono.html` (`<section>` 6/5), `eletricista-disjuntor-disparar-braganca.html` (`<h1>` multiples), `public/blog/blog-problemas-eletricos-inverno.html` (**`<h1>` 25/1**). Ces cinq exigent une lecture au cas par cas — un prologue triplé et 25 `<h1>` ne se réparent pas par un prédicat mécanique. ℹ️ **`blog-problemas-eletricos-inverno.html` porte le MÊME défaut sur ENR (`<h1>` ×26)** : fichier jumeau, défaut jumeau. | R4 (zéro invention — le bloc E-E-A-T est transplanté **verbatim** d'un gabarit unanime à 36/36 et **byte-identique après patch** ; les fermetures de (b) sont imposées par les règles d'imbrication, pas choisies), R11/R12 (contenu absent en production = priorité), R8 (témoins avant/après par fichier, comptés **en Python sur le contenu**, jamais via `git` depuis le sandbox), commit atomique 1 fichier = 1 commit, R6, R7 (zéro merge), R-WT (worktree ; checkout partagé **non touché**) | 13 commits, **12 fichiers de production** + `SEO_PLAN.md`. **Témoins R8** — famille (a), identiques sur les 6 : `<main>` **1/0 → 1/1** · `<section>` **1/0 → 1/1** · `<div>` **5/5 → 6/6** · bloc E-E-A-T byte-identique au donneur **False → True** · liens internes de la section **0 → 5** par page (**+30 au total**). Famille (b), identiques sur les 6 : `<body>` **1/0 → 1/1** · `<div>` **2/1 → 2/2**. **Déséquilibres résiduels sur les 12 fichiers : 0** (contrôle final sur 7 balises). Balayage global : **2 398** fichiers HTML, **1 seul** sans DOCTYPE et sans `<html lang>` (`contactos.html`, **pris par une PR ouverte** — même défaut exact que sur CU, corrigé là-bas par la PR #273). Branche `loop/2026-08-23-eu-structure` depuis `origin/main`, en **worktree** sous `~/work/Sites/_worktrees/loop-eu-20260823`. | ⏳ PR ouverte |
...
- Vague 1 strictement limitée à **95 fichiers HTML / 190 remplacements** du motif homogène `atendimento • 24h/7d`; aucun autre motif R145 n'a été modifié dans cette vague.
...
- **Fix R145 (verrouillé 28/06/2026)** : 5 chaînes «mediante confirmação | após confirmação por telefone | prioridade absoluta» purgées :
  - hero `<p>` «atendimento.» -> «Atendimento por telefone.»
  ...
  - card «Tempo de resposta médio: atendimento» -> «atendimento por telefone»
...
- **Fix R145 (verrouillé 28/06/2026)** : 4 chaînes «mediante confirmação | após confirmação por telefone | prioridade absoluta» purgées :
  - hero `<p>` «atendimento.» -> «Atendimento por telefone.»
  - card «Avaria com cheiro a queimado» «Diagnóstico após confirmação por telefone + reparação.» -> «Diagnóstico após contacto telefónico + reparação.»
  - card prix «Tempo de resposta médio: atendimento» -> «atendimento por telefone»
...
| **R145 (2)** | `atendimento` (tempo resposta) | `24h/7d — incluindo domingos e feriados. Orçamento por escrito antes da deslocação.` |
...
- **MAIS** sweep R145 + source-of-truth align a révélé **2 incohérences réelles** corrigées sur le même commit `cec7a833f` :
  - **R145** (verrouillée 28/06/2026 par Philippe) : 4 occurrences « atendimento » + 1 « prioridade absoluta » + 1 « Tempo de resposta médio » → remplacements neutres alignés sur PR #235 (t_71c207e4) + #238 (t_df870168) : « atendimento por telefone • 24h/7d » / « atendimento por telefone, conforme zona e disponibilidade » / « Emergências priorizadas ».
