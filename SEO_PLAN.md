...
| 2026-08-05 | Hermes (Kanban `t_391ee4ba`) | **Conformidade EU Murça — scope-electric-on-plumbing FPP réfuté + R145 + Z4/45€** | `c180a5bd7` empilé sur `fix/eu-conform-xinzo-scope-r145-z4-t_b1cb089e` (PR #251 cumul 10 fixes). `eletricista-fuga-corrente-murca.html` (1 fichier, +1/-1 sur 1 mega-ligne ; ~7 chaînes modifiées). **scope-electric-on-plumbing FAUX-POSITIF réfuté** (page 100% élec : fuga de corrente → diferencial, curto-circuito, Megger MFT1741+, Fluke T6-1000, ROLeak Aqua 3Plus acoustique, FLIR E96, câmara 30 m, DGEG TRIESP 90062, NAP 932 321 892 — collision lexicale «fuga de corrente» élec vs «fuga de água» plomberie documentée pattern FP moucos/cumieira/torre-de-moncorvo/vidago/ervedosa-do-douro/etc.). Aucune section `<unique-urg-can>` plomberie — les 4 occurrences `canalizador` sont dans JSON-LD sameAs (backlinks cross-site Annexe A conformes). **Fix source-of-truth Z4/45€** (precos-zonas.json : Murça = Z4, pas Z3 comme l'héritage) : zone-badge `Zona 3` → `Zona 4` ; card prix `Deslocação Zona 3: 35€ (já incluída no preço)` → `Deslocação Zona 4: 45€ (sob orçamento por escrito)` ; FAQ custo `para Zona 3` → `para Zona 4` ; FAQ tempo `Para Zona 3` → `Para Zona 4` (Z3+35€ → Z4+45€ uniforme). **R145 purge 6 chaînes** : hero sub `atendimento mediante confirmação por telefone • 24h/7d` → `Atendimento por telefone • 24h/7d` ; card prix `Tempo de resposta médio: atendimento mediante confirmação por telefone` → `Atendimento: por telefone, 24h/7 dias — chamada confirmada antes da deslocação` (R145 délai chiffré INTERDIT) ; FAQ custo `Zona 3` → `Zona 4` ; FAQ tempo `Para Zona 3, atendimento é mediante confirmação por telefone. Em emergências, prioridade absoluta.` → `Para Zona 4, atendimento por telefone, 24h/7 dias. Em emergências, chamamos com prioridade.` ; cta-bottom sub `atendimento mediante confirmação por telefone • 24h/7d` → `Atendimento por telefone • 24h/7d` ; zone-badge `Zona 3` → `Zona 4`. Doctrine §12 intacte (70€/h ×1, Z1-Z6 grille ×1, +50% ×1, orçamento por escrito ×4, NAP 932 ×7, DGEG 90062 ×1, équipement Fluke/Megger/ROLeak/FLIR/câmara 30m intact, JSON-LD sameAs backlinks cross-site conformes Annexe A). | t_391ee4ba, AGENTS.md §11+§12+§13+§14+§15, PRICING.md, precos-zonas.json. Pattern : PR #251 vidago (9be42a90d t_4473352e) + lagoa (e944e9927 t_0f62f93a) + lordelo (6a1c38e80 t_ba8931c3) + britiande (ba8a389cb t_f278aca2) + resende (71dc8ea56 t_5e313cb6) + torre-de-moncorvo (cac587ea5 t_493df2f1). R7 PR draft, NE PAS merger sans GO Filipe. | Témoin grep post-patch : 0 R145 (mediante confirmação / prioridade absoluta / 35€ / 45€+Z4 source-of-truth aligné), 0 scope-plomberie HTML, 0 «Canalizador de Urgência» heading, 4× «Zona 4» (badge + card + FAQ custo + FAQ tempo), doctrine §12 intacte. | ⏳ PR draft — attente review/merge Philippe |
