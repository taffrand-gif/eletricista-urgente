...
| 2026-08-13 | Hermes (Kanban `t_d71382e9`) | **[CONFORMIDADE-URGENT] eu : scope-electric-on-plumbing fp réfuté + R145 purge 4 chaînes on eletricista-fuga-corrente-sendim.html (PR #282 DRAFT)** | `eletricista-fuga-corrente-sendim.html` (page pilier/cluster Sendim Z4 = 45 €). Pool-keeper signal `scope-electric-on-plumbing` = **FP réfuté** : page 100% élec (Fuga de Corrente / Diferencial dispara / ROLeak acoustique / Fluke T6-1000 / Megger MFT1741+ / FLIR E96 / câmara 30 m / DGEG TRIESP 90062 / NAP 932 321 892). 6 occurrences `canalizador` toutes dans JSON-LD `sameAs` (backlinks cross-site conformes Annexe A). Pattern FP déjà documenté (t_24099f7e / t_bf6a4791 / t_7ec530ae) — collision lexicale «fuga de corrente» (élec R12) vs «fuga de água» (plomberie). **Fix réel appliqué — R145 purge 4 chaînes** (verrouillé 28/06/2026) : (1) hero `<p>atendimento mediante confirmação por telefone • 24h/7d</p>` → `<p>Atendimento 24h/7d</p>` ; (2) card prix `Deslocação Zona 4: 45€ (já incluída no preço) + Tempo de resposta médio: atendimento mediante confirmação por telefone` → `Deslocação Zona 4: 45€ (sob orçamento por escrito)` (suppression ligne mensongère «já incluída no preço» + suppression «Tempo de resposta médio: atendimento mediante confirmação por telefone») ; (3) FAQ `Para Zona 4, atendimento é mediante confirmação por telefone. Em emergências, prioridade absoluta.` → `Para Zona 4, chegada conforme disponibilidade da agenda. Confirmamos sempre o horário antes de nos deslocar.` ; (4) cta-bottom `<p>atendimento mediante confirmação por telefone • 24h/7d</p>` → `<p>Atendimento 24h/7d</p>`. Branche `fix/eu-conform-sendim-r145-z4-t_d71382e9` forké depuis `main@9c2f99ef3`, 1 commit atomique `1e00a5faa` `fix(eu,conform): scope-electric-on-plumbing fp réfuté + R145 purge + Z4/45€ source-of-truth align on eletricista-fuga-corrente-sendim.html (t_d71382e9)`, **PR DRAFT #282** ouverte sur `github/taffrand-gif/eletricista-urgente`, 1 fichier modifié, 1 ligne physique touchée (fichier monolithique 1 ligne), 4 substitutions sémantiques dans le body. **Doctrine R12 §12 Transparência Radicale intacte** : 70 €/h × 1, Z1-Z6 grille × 1, +50 % majoration × 1, orçamento por escrito × N, NAP 932 321 892 × 10 (7 texte + 3 href), NAP 928 = 0, Zona 4 × 4, DGEG TRIESP 90062 × 2 (bloc + JSON-LD Person/Org), Fluke T6-1000/Megger MFT1741+/ROLeak Aqua 3Plus/FLIR E96/câmara 30 m (intacts). **R11 ZÉRO INVENTION respectée** : aucun prix/zone/délai/chantier inventé. **Source-of-truth alignée** : PRICING.md Z4=45 € + precos-zonas.json `Sendim=4` ✓. **Gates** : R7 respecté (STOP validation Philippe avant merge = PR DRAFT, aucun merge créé). R3 audit lecture-seule : recompte live post-patch 0 leak R145. | PR #282 DRAFT ouvert, `git diff --check` vert, scope strict 1 fichier, **0 hit R12 INTERDIT** (`mediante confirmação` / `após confirmação por telefone` / `prioridade absoluta` tous à 0), Z4=45€ aligné, doctype intact. | 🛑 STOP R7 — PR DRAFT #282 en attente GO Filipe |
