...
|---|---|---|---|---|---|---|
| 2026-07-18 | cowork-loop | **IndexNow J1 — régénération eletricista-urgente-indexnow-urls.txt (liste stale)** | Constat : `indexnow-key.txt` + fichier de vérification `indexnow-<key>.txt` existent déjà (commit `9142d3067`, 03/07, M10) — la note "❌ Pas de fichier indexnow-key.txt" de la section ÉTAT POST-FOURNÉE 17/07 est erronée/obsolète. En revanche `eletricista-urgente-indexnow-urls.txt` (racine) était réellement stale depuis 03/07 (1829 URLs) : homepage `https://eletricista-urgente.pt/` absente, 185 URLs sitemap manquantes (villages P1C batch #153/#156 fusionnés 17/07), 25 URLs blog obsolètes (pages supprimées purge R11) encore présentes. Régénéré par union triée `sitemap.xml` (45 URLs) + `sitemap-villages.xml` (1944 URLs) = **1989 URLs dédupliquées**. 1 fichier, 1 commit `9c1f9185c`, branche `loop/2026-07-18-eu-indexnow-urls-refresh`. **Soumission API réelle (POST api.indexnow.org) NON faite** — action infra hors scope loop code (AGENTS.md R1 double confirmation), fichier prêt pour GO Philippe. | Tâche J1 2026-07-18 planifiée section ÉTAT POST-FOURNÉE 2026-07-17 ligne "Queue IndexNow J1". R8 (grep avant/après), R4 (zéro invention — pas de nouvelle URL inventée, uniquement miroir sitemap existant). | Grep avant : homepage présente = 0, wc -l = 1829. Grep après : homepage présente = 1, wc -l = 1989, stale blog URLs = 0. | ✅ Fait (régén. fichier) — ⏳ soumission API attente GO Philippe |
| 2026-07-17 | Hermes | **GEO dedup FAQ curto-circuito (no sitemap patch, EU déjà conforme audit §3.2)** | `curto-circuito.html` : suppression 2 FAQ doublonnées (DOM + JSON-LD synchronisés) — #2 "O que provoca... numa casa em TM?" et #5 "Como sei se é curto-circuito ou apenas sobrecarga?" — au profit de leurs versions #9/#10 plus complètes (ajoutent angles locaux Trás-os-Montes : granito, oxidação, observação 10 min, fuite). Count AVANT=11/11 (CSS=JSON), APRÈS=9/9 (DOM=JSON, alignement parfait vérifié). 0 référence cassée, LocalBusiness+Service intacts (70 EUR conservé). Sitemap.xml NON touché (45 URLs déjà conforme, audit `_audit/SITEMAP-CORE-PILIERS-2026-07-17.md` §3.2 —'aucun patch requis'). falha-energia NON touchée (82/100, 'ne touche pas sauf reco explicite'). Claim inventé = 0, délai chiffré = 0 (R145). | Audit `_audit/GEO-CITABILITE-PILIERS-2026-07-17.md` corto-circuito 76/100 'redondance détectée' + mission EU dedup. DoD = sitemap grep OK + FAQ count avant/après + JSON-LD OK + claims 0. | 9 FAQ DOM = 9 FAQ JSON, sitemap inchangé, branch `feat/monopole-piliers-eu` 1 commit à pusher, 0 merge main (AGENTS.md §7). | 🟡 À pusher + attendre GO merge |
