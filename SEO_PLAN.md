...
- 🔴 **R12 délais inventés** : ~896 pages « resposta em X min » + ~1884 « resposta prioritária / mediante confirmação » (R145) ← gros chantier doctrine
  - **Recompte live 17/08/2026 19:42 BST (t_46006a18, R01 leçon #447)** :
    - `resposta em X min` = **33 fichiers / 5 hubs concelhos** (`alijó`, `boticas`, `ribeira-de-pena`, `vila-flor`, `alfandega-da-fe`) — **claim initial ~896 largement caduque**, purgé en A2-BIS + A4 + sub-agent `t_bc868eec` (PR #248) + R145 vagues (03/08 t_0d3bd888).
    - `Resposta prioritária` = **0 fichier** — **gisement 1884 entièrement refermé** par A4.
    - `mediante confirmação` = **1569 fichiers / 2734 occurrences** — **dette M0 verrouillée** ligne 22 (STOP Filipe tranchage « purger ou tolérer + MAJ doctrine »).
    - `Quanto tempo demoram a chegar` = **956 fichiers** (gisement ligne 248 inventaire FAQ 13/08, 4 variantes dont 428 « min conforme zona » orphelin grammatical) — **chantier parent distinct non démarré**.
  - **PR DRAFT #307** (t_46006a18) : 1 fichier prototype `concelhos/alfandega-da-fe.html` ligne 24 og:description purgée. GO nominatif Filipe avant batch sur les 4 hubs restants (AGENTS.md §14 R12 §validation).
...
| 2026-08-17 | Hermes (Kanban `t_46006a18`) | **[CONFORMITÉ-EU] SEO_PLAN ligne 108 : R12 délais inventés — recompte live + 1 PR prototype #307 sur résidu 33 fichiers / 5 hubs concelhos** | Recompte live 17/08 19:42 BST (R01 leçon #447 + R11 contrôle positif worktree) = `resposta em X min` **33 fichiers / 5 hubs concelhos** (`alijó`/`boticas`/`ribeira-de-pena`/`vila-flor`/`alfandega-da-fe`) tous en `og:description` ligne 24 · `[Rr]esposta [Pp]riorit` **0 fichier** (gisement 1884 entièrement refermé par A4) · `[Mm]ediante [Cc]onfirma` **1569 fichiers / 2734 occ** (dette M0 verrouillée ligne 22 STOP Filipe) · `Quanto tempo demoram a chegar` **956 fichiers** (gisement ligne 248 chantier parent non démarré). **Conclusion** : claim SEO_PLAN ligne 108 « ~896 + ~1884 » **largement caduque** au 17/08 — purgé par A2-BIS + A4 + sub-agent `t_bc868eec` (PR #248) + R145 vagues 03/08. **Action** : 1 PR prototype `fix/eu-prototype-og-description-q3-resposta-X-min-t_46006a18` créée depuis `origin/main@03f693c85` → commit `dc80f6ddc` sur `concelhos/alfandega-da-fe.html` (1 fichier, +1/-1, scope strict ligne 24 `og:description`) → push OK → **PR DRAFT #307 ouverte** : `resposta em 31 min` → `orçamento por escrito, deslocação 35€. Atendimento 24h/7d, chegada conforme disponibilidade`. R145 conforme (« 24h/7d » OK + « chegada conforme disponibilidade » OK). Doctrine §12 intacte (NAP 932 = 8, 70€/h = 8). **Dette connexe identifiée HORS scope** : (1) `meta description` l.17 des 5 hubs contient `(~31 min)` / `(~89 min)` etc. — délai chiffré déguisé en estimation kilométrique ; (2) FAQ body l.83 (alfandega-da-fe) contient `chegamos em 32 km (~31 min de viagem desde a base)` + fourchettes inventées `a partir de 90€ na 1ª hora, 155€ em 2h` — R11+R12 ; (3) même chaîne `og:description` ligne 24 sur les 4 autres hubs (`alijó`/`boticas`/`ribeira-de-pena`/`vila-flor`). AGENTS.md §14 R12 §validation : **AUCUN batch avant GO nominatif Filipe sur PR #307**. Leçon #483 (worktree double-comptage) appliquée. Reprend pattern PR #229 (t_869cc997) + #258 (t_0dd0259b) + #292 (t_ee0a8d1d) + #297 (t_ff4dd9e1). LECONS.md leçon #486 consignée : **« Recompter live avant d'agir sur un claim SEO_PLAN historique — R01 stricte. »** | R01 + R02 + R04 + R11 (recompte live + contrôle positif worktree + exclusion .worktrees) | **PR DRAFT #307** https://github.com/taffrand-gif/eletricista-urgente/pull/307 — 1 commit `dc80f6ddc`, 1 fichier patché, scope strict. SEO_PLAN ligne 108 mise à jour avec recompte live + dette connexe documentée. | ⏳ PR #307 DRAFT — STOP merge R7, GO Filipe obligatoire + décision dette connexe |
| 2026-08-17 | Hermes (Kanban `t_58988cb3`) | **[CONFORMITÉ-EU] SEO_PLAN lignes 116-121 : Homepage squelettique — NO-OP légitime 5/6 claims résolus post PR #33+#35+#36 (29/06)** | Recompte live 17/08 19:50 BST (R01 leçon #447 + R11 contrôle positif worktree `index.html`) — 6 claims initiaux : **(L116) Homepage squelettique 16-39 éléments** = `wc -l index.html`=290 / éléments structurants `<h1|<h2|<section|<article|<li|<div class="card|service)=19` → **DANS la fourchette 16-39** mais pas squelettique : 9 `<section>`, 7 `<h2>`, 1 `<h1>`, schema riche (FAQPage/Organization/Person/EducationalOccupationalCredential/City/AdministrativeArea/GovernmentOrganization). **(L117) Grille 70€/h + Z1-Z6** = `70 €/h`×4 + `70€/h`×1 + Z1×5/Z2×4/Z3×6/Z4×5/Z5×3/Z6×6 — **claim RÉSOLU** par PR #33+#35 (29/06, A1/A2 Doctrine §12). **(L118) « fala sempre com a mesma pessoa, não um call center »** = 0 occurrence littérale MAIS section §4 EQUIPA LOCAL l.126-128 contient `Sem call center, sem plataforma anónima.` + `Falamos consigo diretamente, sem intermediários nem call center.` + `O Filipe Bragança (Staff-Seekers / Norte Reparos) atende diretamente… É eletricista de formação, com experiência em instalações…` — **équivalent sémantique vérifié**, formulation canonique R12 §2 « fala sempre com a mesma pessoa » NON servie verbatim. **(L119) Équipement réel (Fluke/Megger/FLIR)** = `Fluke T6-1000`×1 + `Megger MFT1741`×1 + `FLIR E96`×1 + `ROLeak`×1 — **claim RÉSOLU** PR #33 (29/06). **(L120) FAQ honnête** = `<details|faq`×18 + `(1h)` × occurrences multiples — **claim RÉSOLU** PR #33 (29/06). **(L121) schema.org FAQPage** = `"@type":"FAQPage"`×2 + `Question`×4 + `Answer`×4 — **claim RÉSOLU** PR #33 (29/06). **Verdict** : 5/6 RÉSOLUS (L117/L119/L120/L121 + L116 INFLÉCHI = conforme) ; L118 = variante rédactionnelle acceptable (équivalent sémantique ligne 126-128), pas d'invention (R11), pas d'action batch (AGENTS.md §14 R12 §validation). **Action retenue** : **0 PR, 0 fichier HTML modifié, 0 commit code, 0 push**. Tâche consignée + clôturée. | R01 (recompte live AVANT action) + R04 (NO-OP légitime documenté) + R11 (zéro invention — variante rédactionnelle ≠ invention) + R7 (zéro merge sans STOP nominatif) | 1 ligne SEO_PLAN ajoutée (section 🔄 HISTORIQUE, append-only après entrée t_46006a18). 0 HTML modifié, 0 commit code, 0 PR créée, 0 merge, 0 push. Leçon #447 confirmée. | 🛑 NO-OP — chantier lignes 116-121 caduque post-29/06 (PR #33+#35+#36), aucun patch requis. Variante rédactionnelle L118 signalée pour backlog (micro-PR 1 fichier si Philippe souhaite alignement verbatim R12 §2) |
