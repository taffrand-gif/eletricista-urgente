...
| 2026-08-05 | Hermes (Kanban `t_ee6a3975`) | **[CONFORMIDADE-URGENT] eu : wrong-phone em eletricista-urgente-alij.html** | Fix unitaire sur `eletricista-urgente-alij.html`, empilé sur `fix/eu-conform-xinzo-scope-r145-z4-t_b1cb089e` → PR draft #251. Le contrôle public de l'URL demandée suit 308 `/eletricista-urgente-alij.html` → 301 `/eletricista-urgente-alijo` → 200 : le live canonique `alijo` est déjà corrigé, mais le fichier source ciblé `alij.html` contenait encore la section `<unique-urg-can>` plomberie et la chaîne R145 «mediante confirmação por telefone». Section convertie au pattern élec déjà validé dans ce repo : label `Eletricista de Urgência`, cinq symptômes élec existants, consigne `desligue o disjuntor geral no quadro`; R145 neutralisé en `Atendimento por telefone` / `Chegada conforme disponibilidade`. | Le téléphone `932 321 892` était déjà le NAP élec correct ; le type `wrong-phone` signalait en réalité le contexte plomberie (`Canalizador`, `Fuga ativa`, `Cano rebentado`, `Esgoto`, `torneira`, `esquentador`). R11/R12/R145 respectées ; aucun prix, zone, délai ou service inventé ; `precos-zonas.json['Alijó']=5` vérifié sans introduire de zone locale dans cette page. | Témoins post-fix : 0 `unique-urg-can`, 0 `Canalizador de Urgência`, 0 `feche a torneira`, 0 `Fuga ativa/Cano rebentado/Esgoto/Válvula`, 0 `mediante confirmação/Resposta a confirmar`; NAP 932, grille 70 €/h + Z1-Z6 + +50 % et DGEG 90062 intacts. | ⏳ PR draft #251 — NE PAS merger sans GO Philippe (R7) |
