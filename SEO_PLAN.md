...
| 2026-08-13 | Hermes (Kanban `t_ee0a8d1d`) | **[CONFORMIDADE-URGENT] eu : scope-electric-on-plumbing corrigé + invented price 110 remplacé par PriceSpecification Z6=65€ on eletricista-fuga-corrente-amarante.html (PR #292 DRAFT)** | `eletricista-fuga-corrente-amarante.html` (Amarante Z6 per `precos-zonas.json` = 65 € deslocação) — Pool-keeper signal `scope-electric-on-plumbing` **VRAI** (vs t_0dd0259b FPP). 3 JSON-LD sameAs arrays (Service Fuga Corrente, Service Eletricista Urgente, LocalBusiness openingHours) listaient `canalizador-norte-reparos.pt` + `canalizador-urgente.pt` sur page 100 % élec. Retirés, remplacés par les 2 URLs marque-élec `[eletricista-norte-reparos.pt, eletricista-urgente.pt]` (pattern identique à commits 86003bf12, 520e2884e, b03802f80). `offers.price='110'` inventé (hors PRICING.md : 70 €/h + Z6=65 € + mínimo faturado) → remplacé par `PriceSpecification` description citant PRICING verbatim (70 €/h, Z6=65 €, +50 % majoration, mínimo faturado). Branche `fix/eu-amarante-scope-electric-t_ee0a8d1d` depuis `origin/main@86003bf12` (HEAD récent post merge canalizador-* purge), commit `ce2ab4923`, **PR DRAFT #292** `state=OPEN isDraft=true`, +2/-2 sur 1 fichier (taille nette identique). Preuve live : `curl -IL https://eletricista-urgente.pt/eletricista-fuga-corrente-amarante.html` → 308 → 200 sur `/eletricista-fuga-corrente-amarante` ; `grep sameAs` AVANT = 3×3 canalizador, APRÈS = 3×2 élec. Race condition workspace signalée : le pool-keeper dispatche plusieurs tâches concurrentes sur le même `workspace_path`, j'ai perdu un 1er commit (la branche `fix/eu-amarante-scope-electric-20260813` a été swappée par un checkout concurrent vers `fix/eu-sao-cosmado-scope-electric-r145-t_2cf0b2d5`, mon commit a été écrasé par un reset). Mitigation appliquée : checkout main → re-apply patches → commit immédiat + push avant tout autre switch. À signaler au pool-keeper pour durcissement (lock par branche, pas par workspace_path). | Doctrine §12 (Transparência Radicale) intacte : 70 €/h ×6, Z6=65 € ×1, +50 % ×1, mínimo faturado ×1, NAP 932 321 892 ×2, 0 NAP canal 928, équipement intact, JSON-LD valid (6/6 parse OK post-fix), pas de service plomberie ajouté, pas de témoignage/chantier inventé. **0 résidu grep post-fix** : canalizador refs 6→0, price 110 1→0, priceSpecification 0→1. R7 strict : PR DRAFT #292 attente GO nominatif Filipe (AGENTS.md §1 STOP validation + §7 pas de merge sans GO). | 1 fichier patché `eletricista-fuga-corrente-amarante.html`, 0 résidu, doctrine §12 intacte, NAP 932 préservé, DGEG TRIESP 90062 untouched, JSON-LD 6/6 valide, HTML balanced. Race condition workspace signalée au pool-keeper. | ⏳ PR DRAFT #292 — attente GO merge Philippe (R7) |
