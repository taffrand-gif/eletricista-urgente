...
| 2026-08-13 | Hermes (Kanban `t_09a01604`) | **[CONFORMIDADE-URGENT] eu : scope-electric-on-plumbing corrigé on eletricista-fuga-corrente-vila-franca-das-naves.html (PR #295 DRAFT)** | Branche `fix/eu-vila-franca-das-naves-scope-electric-t_09a01604` depuis `origin/main@9c2f99ef3`, 1 commit `75b4b0fa6` (cherry-pick de `2a7b97db7` race-récupéré sur branche collègue `t_93ffe04a` São Martinho de Mouros). 3 références `canalizador-*` (CNR + CU) retirées des 3 blocs JSON-LD sameAs (Service Fuga Corrente Vila Franca das Naves, Service Eletricista Urgente, LocalBusiness openingHours), remplacées par les 2 refs brand-electric [eletricista-norte-reparos.pt, eletricista-urgente.pt]. `offers.price="110"` inventé remplacé par PriceSpecification citation PRICING.md verbatim (70 €/h + Z6=65 € + majoração +50 % + mínimo faturado). 1 fichier, 4+/3-. Preuve live HTTP 200 sur `eletricista-fuga-corrente-vila-franca-das-naves` (308→extensionless, 4 grep violations confirmées). Récupération race condition workspace pool-keeper : commit parti sur branche collègue São Martinho de Mouros (stash collègue préservé via `git stash` + reset collègue propre + cherry-pick sur ma branche = 0 contamination croisée). R7 STOP Filipe avant merge. R11 zéro invention (Z6 confirmé precos-zonas.json, Vila Franca das Naves = 960 localités). AGENTS.md §12 R12 + PRICING.md 70€/h respectés. JSON-LD 6 blocs valides post-patch. | AGENTS.md §12 R12 (transparence), PRICING.md 70€/h + Z6=65€ + mínimo faturado, precos-zonas.json (Vila Franca das Naves=Z6 vérifié), JSON-LD 6/6 valides | ⏳ PR #295 DRAFT — attente GO merge Philippe (R7) |
