...
| 2026-09-24 | Hermes (t_7b290488) | **[CONFORMIDADE-URGENT] eu wrong-phone eletricista-aguas-vivas — KO RÉFUTÉ (close-on-arrival, scan faux-positif)** | Brief `t_7b290488` (scan local « wrong-phone » sur `eletricista-aguas-vivas.html`, extrait `ign:center;background:#0a4d68;color:#fff…` non vérifié sur disque) = **même classe de faux-positifs** que `t_c6432a28` (cnr/salzedas), `t_ac479308` (cnr/marialva), `t_1662f744` (cu/alvendre), `t_c0beada0` (eu/amendoeira), `t_4fa3d40f` (cu/almendra), `t_9ce7c4c5-mezquita-cu` : signal `HTTP code = None` ou « wrong-phone » détecté en local, réfuté en HTTP 200 stable et NAP conforme. **Vérifications live (R02)** : (a) `sha256 local = sha256 live` (`cb4efa019bbb8497303e445229a301fbbfcd550f1e2e9564bd3968c94d95f2ae`) ; (b) téléphone exhaustif : `+351 932 321 892` = 12, `+351****1892` (tel: href masqué cf. PRICING.md §NAP) = 7, `+351-932 321 892` (JSON-LD) = 2, `932 321 892` = 1, **928 = 0** ; (c) ETag Vercel `b2b43040a919e8bc9528a18ef2a20d4d` stable depuis 14/09 20:47 UTC ; (d) extrait cité inexistant : 0 hit `ign:center;background:#0a4d68` ; (e) absence contamination plomberie : 0 hit `Fuga ativa`/`Inundação`/`Cano rebentado`/`Válvula de segurança`/`feche a torneira geral` ; (f) JSON-LD `LocalBusiness.telephone = "+351 932 321 892"` ✅. **Décisions worker `t_7b290488`** : (1) **PAS de patch wrong-phone** — fichier conforme ; (2) **PAS de correction template-bleed** — contamination absente ; (3) **PAS de merge** — R7 « 0 merge sans GO » ; (4) rapport `_audit/SMKO-eu-2026-09-24-aguas-vivas.md` créé ; (5) PR draft `fix/eu-conformidade-wrong-phone-aguas-vivas-t_7b290488` à ouvrir. Défauts **secondaires** (hors-porte) : doublons `<head>` + « Zona 3 »/« Zona 4 » affichées en parallèle — à batcher avec GO selon AGENTS.md §12 R12 §validation. | ✅ Fait (rapport créé + PR draft à ouvrir) |
| 2026-09-24 | Hermes (t_c8d2e8f1) | **[CONFORMIDADE-URGENT] eu wrong-phone eletricista-urgente-vale-de-prados — faux-positif NAP + corrigé template-bleed** | Brief `t_c8d2e8f1` (scan local « wrong-phone » sur `public/eletricista-urgente-vale-de-prados.html`, extrait `feche a torneira geral de água … ligue 932 321 892`). **Vérifications pre-fix (R02)** : (a) NAP exhaustif : `932 321 892` = **6**, `928 484 451` = **0** → NAP lui-même 100 % élec conforme ; (b) **vrai défaut** = `unique-urg-can` (lignes 86-97) contient gabarit plomberie hérité : items *Fuga ativa / Inundação / Cano rebentado / Esgoto a transbordar / Válvula de segurança* + consigne *fechar torneira geral de água* — l'analyseur a classé « wrong-phone » car la consigne d'urgence ne correspond pas au métier d'appel. **Correctif appliqué** : patch chirurgical **byte-identique au précédent `t_e6bfd650` corticos (commit `e24f1cb00`)** — 6 lignes modifiées (5× `<li>` + 1× `<p>`), 100 % électricité aligné AGENTS.md §13 gabarit : *Curto-circuito ativo / Sem luz total ou parcial / Tomada ou quadro a aquecer / Choque elétrico / Disjuntor diferencial (30 mA) que dispara em seguida* + consigne *desligar o disjuntor geral (QGD) e ligue 932 321 892 — eletricista certificado DGEG n.º 90062. Orçamento por escrito antes de qualquer intervenção, sem surpresas.* **Télémétrie post-fix** : NAP 932 = 6, 928 = 0, vocabulaire plomberie (`torneira|água|cano|esgoto|válvula`) = **0**, `Curto-circuito` = 2, `DGEG n.º 90062` = 1, `sem surpresas` (R12 §1) = 2, diff +6/-6. **Décisions worker `t_c8d2e8f1`** : (1) worktree `fix/eu-conformidade-vale-prados-plumbing-bleed-t_c8d2e8f1` créé depuis `main` (HEAD `54f389eb1`) ; (2) commit `06bdd4fbc` sur 1 fichier ; (3) push `origin` OK ; (4) **PR draft #396** ouverte (isDraft=true, base=main) — **GO Filipe obligatoire avant merge (R3+R7)** ; (5) rapport `_audit/SMKO-eu-2026-09-24-vale-de-prados.md` créé. **Hors scope** (lot hérité ~25k violations R12) : meta « 30 € » → Z1=15 €, JSON-LD FAQ « 100 €/h » → 70 €/h, services `Desde €35*/45*/55*` → R12 §1 interdit, Garantia 2 anos / 1 ano incohérence footer. Ces points = batcher avec GO selon R12 §validation. **Hotspot à signaler** : même template-bleed existe sur d'autres pages `public/eletricista-urgente-*` non encore corrigées (cf. corticos commit message : *vila-nova-de-foz-coa, penedono, lazgrim, alfandega-da-fe, vimioso, sao-joao-de-tarouca, ribeira-de-pena, britiande, lordelo, lagoa, olmos*…) — batch à GO. | ⏳ En cours (PR DRAFT) → ✅ après GO Filipe |
| 2026-09-24 | Hermes (t_7c7c490f) | **[CONFORMIDADE-URGENT] eu wrong-phone eletricista-urgente-podence — faux-positif NAP + corrigé template-bleed** | Brief `t_7c7c490f` (scan local « wrong-phone » sur `public/eletricista-urgente-podence.html`, extrait `feche a torneira geral de água … ligue 932 321 892`). **Vérifications pre-fix (R02)** : (a) NAP exhaustif : `932 321 892` = **9**, `928 484 451` = **0** → NAP lui-même 100 % élec conforme ; (b) **vrai défaut** = `unique-urg-can` (lignes 191-202) contient gabarit plomberie hérité identique à corticos (#392) et vale-de-prados (#396) : items *Fuga ativa / Inundação / Cano rebentado / Esgoto a transbordar / Válvula de segurança* + consigne *fechar torneira geral de água* — l'analyseur a classé « wrong-phone » car la consigne d'urgence ne correspond pas au métier d'appel. **Correctif appliqué** : patch chirurgical **byte-identique au précédent `t_e6bfd650` corticos (commit `e24f1cb00`)** — 6 lignes modifiées (5× `<li>` + 1× `<p>`), 100 % électricité aligné AGENTS.md §13 gabarit : *Curto-circuito ativo / Sem luz total ou parcial / Tomada ou quadro a aquecer / Choque elétrico / Disjuntor diferencial (30 mA) que dispara em seguida* + consigne *desligar o disjuntor geral (QGD) e ligue 932 321 892 — eletricista certificado DGEG n.º 90062. Orçamento por escrito antes de qualquer intervenção, sem surpresas.* **Télémétrie post-fix** : NAP 932 = 9, 928 = 0, vocabulaire plomberie (`torneira`) = **0**, `Curto-circuito|Sem luz total|disjuntor geral \(QGD\)|diferencial \(30 mA\)` = 5, `DGEG n.º 90062` = 1, `sem surpresas` (R12 §1) = 2, diff +6/-6, SHA local `b6c5908112504f6da4e15bc953fbfaba425c4c08f1ca3f456bd8a84b3265e167`. **Décisions worker `t_7c7c490f`** : (1) worktree `fix/eu-conformidade-podence-plumbing-bleed-t_7c7c490f` créé depuis `main` (HEAD `54f389eb1`) ; (2) commit `81e2f98cf` sur 1 fichier ; (3) push `origin` OK ; (4) **PR draft #397** ouverte (isDraft=true, base=main) — **GO Filipe obligatoire avant merge (R3+R7)**. **Hors scope** (lot hérité ~25k violations R12) : meta « 30 € » → Z1=15 €, services `Desde €35*/45*/65*` → R12 §1 interdit, Garantia 2 anos / 1 ano incohérence footer, meta description tronquée (3 occurrences `, em minutos`). **Hotspot à signaler** : **template-bleed `unique-urg-can` toujours présent** sur 8+ autres pages `public/eletricista-urgente-*` (cf. corticos commit message + t_c8d2e8f1) — batch à GO selon R12 §validation. | ⏳ En cours (PR DRAFT) → ✅ après GO Filipe |
**Dernière MAJ** : 2026-09-24 (t_7c7c490f)
