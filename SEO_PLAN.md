...
| T3-INFO eu GSC | Query `eletricista bragança` pos 11.7 / 20 impr / 0 clics 28j (fenêtre 2026-09-14) | MOY | ⏳ PR DRAFT | t_35f59179 | STOP GO Filipe | Renforce page canonique `eletricista-braganca.html` (sitemap priority 0.7) — la page existait déjà et couvre Bragança, mais le title ne reprenait pas la query exacte. Patch R4 strict (zéro invention) : (1) title `Eletricista Bragança · 70 €/h · Orçamento por escrito (24h/7d)` (60 car, verbatim query + signal prix), (2) meta description 184 car (était 116, tronquée `A partir de 105€ (1h)` qui était un forfait inventé → corrigée R11), (3) `<meta charset>` dupliqué supprimé, (4) `<p>` intro juste sous H1 reprenant `Eletricista em Bragança` verbatim + 70 €/h + Z3=35 €, (5) nouvelle `<section class="query-braganca-2026">` 26 lignes / ~280 mots ajoutés (H2 `Eletricista em Bragança — como encontrar eletricista certificado`, H3 `Quando chamar` + 5 symptômes universels non-liés à un cas client, H3 `Como trabalhamos em Bragança` + 6 étapes, footer avec majoration +50 % + mínimo faturado + maillage interne concelhos voisins), (6) JSON-LD FAQPage 3 → 5 entrées (ajout `Onde encontrar eletricista certificado em Bragança?` orientée DGEG + `Eletricista em Bragança faz serviço ao domicílio?` orientée zone Z3), (7) Z3=35 € vérifié contre `precos-zonas.json` ligne `"Bragança": 3` (= source-of-truth). Témoins AVANT/APRÈS : R11 témoignage/chantier 0=0 ; R12 'je' interdit 0=0 ; R145 délai chiffré 0=0 ; NAP 932 321 892 9 (préservé) ; 70 €/h 3 occ (préservé) ; orçamento por escrito 5 occ (préservé) ; JSON-LD valide (parser Python). Out of scope : Pacotes « A partir de sob orçamento por escrito » wording legacy, drift « Zona 2 / 25€ » (legacy chantier séparé), blocs plomberie (« Fuga ativa », « Esgoto a transbordar ») sur page `eletricista-urgente-braganca.html` (autre fichier, autre chantier). Mesure J+7 via gsc-trajectoire-cron.sh : pos < 4 = win, pos > 10 = rollback possible. R7 : PR DRAFT + STOP merge + GO explicite Filipe. |
<!-- CHANTIERS:END -->
...
- 🟡 **R12 délais inventés** (recompte live 2026-09-14 `t_cd6bf70b`) — claim « ~896 + ~1884 » partiellement obsolète : `resposta em X min` = **0 fichier**, `resposta prioritária` = **0 fichier**, `mediante confirmação` = **4 fichiers** (`distritos/douro.html` + `distritos/tras-os-montes.html` × 2 lignes chacun). Résidu R145 réel vit sur motifs élargis : `imediatamente` 158 occ, `chegada em` 134, `resposta rápida` 152, `em 30 minutos` 43, `atendimento imediato` 72, `6 min\b` 70 — **249 fichiers uniques**. Chantier vivant sur sa propre file = PR #376 DRAFT (`fix/eu-r145-concelhos-eta-t_8a7aa083`) purge ETA « min de viagem » sur Bragança. **0 PR draft à ouvrir ici** : doublonnerait PR #376 + flux R145/R12 vivant (X-R12 vagues 2+). Suivi via PR #376 + futures vagues X-R12.
...
| 2026-09-14 | Hermes (t_cd6bf70b) | **NO-OP APPLICABLE ligne 127 — re-vérification R12 délais inventés (recompte live, claim « ~896 + ~1884 » partiellement obsolète)** | Brief désignait « SEO_PLAN.md ligne 127 🔴 R12 délais inventés ~896 + ~1884 (R145) ». **Re-vérification live AVANT action (R4 + leçon #a6f0ecf7)** sur `origin/main@1b572563cf` (2217 fichiers HTML) : (1) `resposta em X min` motif exact de l'énoncé → **0 fichier / 0 occ** ; (2) `resposta prioritária` → **0 fichier / 0 occ** ; (3) `mediante confirmação` → **4 fichiers / 4 occ** seulement (`distritos/douro.html:53+88` + `distritos/tras-os-montes.html:53+88`, formulation « cobertura regional mediante confirmação » sémantiquement OK = *géographique, pas délai*) ; (4) motif réel du résidu R145 = `imediatamente` 158, `resposta rápida` 152, `chegada em` 134, `atendimento imediato` 72, `6 min\b` 70, `em 30 minutos` 43 → **249 fichiers uniques** (bien loin des 2780 cumulés de 2026-08-17, qui était déjà obsolescent vs l'état main actuel). **Décision = NO-OP APPLICABLE** : (a) **0 PR draft à ouvrir** = doublonnerait PR #376 DRAFT (`fix/eu-r145-concelhos-eta-t_8a7aa083` qui purge « min de viagem » sur Bragança) + flux R145/R12 vivant sur sa propre file (X-R12 vagues 2+, registre fixes ENR, X-GAR-6M `t_382e02fb`/`t_44969a2b`/`t_b81e053a` déjà mergés) ; (b) **ligne 127 recompte honnête** : 🔴→🟡 + décomposition motifs large, référence PR #376 + leçon #a6f0ecf7 ; (c) **chantier vivant** = suivi via PR #376 + futures vagues X-R12 (responsable = autres tâches du pool, pas moi). **LEÇON #t_cd6bf70b** : un brief pointant un 🔴 critique dans un mémo vivant peut (a) être déjà obsolète par les passes antérieures sans qu'un recompte ait MAJ la ligne, OU (b) cibler un motif qui a déjà été absorbé mais où la formulation reste. La règle d'or = recompte daté avant toute ouverture PR = recompte > naïveté du chiffre cité. Sœur de t_a6f0ecf7 (ligne 70 ~25k → 2800-3500) + t_2cd5aea6 + t_baaa6498 (ligne 257 NO-OP 2ᵉ passe). R7 : 0 patch code, 0 PR draft, 0 merge — Juste un recompte + 1 ligne SEO_PLAN.md patchée. | 0 fichier HTML patché, scope strict 1 ligne SEO_PLAN.md, recompte live = source de vérité, PR #376 vivante absorbée le résidu ETA Bragança | ✅ Fait (recompte + patch SEO_PLAN uniquement) |
|**Dernière MAJ** : 2026-09-14 (t_cd6bf70b)
