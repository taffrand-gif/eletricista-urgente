...
| 2026-09-16 | Hermes (t_6616354a) | **GSC gap eu 'eletricista bragan&#xE7;a' pos=11.3 (22 impr/0 clics 28j)** | Renforce page canonique `/public/eletricista-urgente-braganca` (+ 165 lignes diff, +121/-44) pour combler le gap GSC : query exacte `eletricista bragan&#xE7;a` pos=11.3, 22 impressions, 0 clic (fenêtre 28j fermée 2026-09-16). Page canonique identifiée par **sitemap + canonical href + probe HTTP=200** (3 pages Bragança-like coexistent, mais la canonique porte la query + prix + JSON-LD complet). Patchs principaux : (1) **title** « Eletricista em Bragança · 35€ Deslocação + 70€/h (2026) » (60 car, query en début + prix visible) ; (2) **meta description** 165 car avec query + Z3/35€ + 70€/h + NAP + phrase obligatoire « orçamento por escrito antes da intervenção, sem surpresas » ; (3) **H1** « ⚡ Eletricista em Bragança · Urgências 24h » ; (4) **bloc Transparence prix** HAUT (HTML + tableau) — main d'œuvre 70€/h + déplacement Z3 35€ + minimum faturado 105€ + majoration +50% + service certifié DGEG 350€ plancher + 4 engagements (estimativa sem custo, orçamento por escrito, deslocação tabelado Z1-Z6, confirmamos antes de faturar) ; (5) **bloc Quem somos** Norte Reparos · fala connosco · fatura NIF · seguro RC · garantía escrita · DGEG TRIESP n.º 90062 (Lei 14/2015) para instalações até 41,4 kVA — formulation collective « a nossa equipa » (aucune mention Filipe seule) ; (6) **6 sections H2 élec** : Serviços + Sintomas + Causas locais (5 causes spécifiques Bragança/Trás-os-Montes : terra ausente, sem 30mA, surcharge aquecimento, humidade/gelo, quadro antigo) + Como resolvemos (équipement EXACT : Megger MFT1741+, Fluke T6-1000, FLIR E96 + caméra 30m) + Quando chamar (3 urgences critiques) + Prevenção (3 points) ; (7) **FAQ longue traîne 5 questions** : « Quanto custa eletricista em Bragança? » + « Como sei que a fatura é justa? » + « Fazem orçamento antes? » + « É urgente — quanto tempo demoram? » + « Há garantia por escrito? Emitem ficha eletrotécnica? » — answer-first 40-60 mots chacune, balisage schema.org FAQPage + microdata HTML ; (8) **JSON-LD canonique `@graph`** : `@context` correctement fermé (= `https://schema.org`), WebSite + Organization + LocalBusiness (addressLocality Bragança, areaServed City Bragança, openingHoursSpecification 24/7, priceRange €€) + Service + FAQPage (5 entrées) ; (9) **tous les NAP canoniques** = `tel:+351932321892` (sans masque) + 932 321 892 = 4 occurrences ; (10) **CTA double canal** Tel +351 932 321 892 + WhatsApp avec tracking gtag, formulation « fala sempre com a mesma pessoa » (neutre, conforme gabarit §13 — à voir si arbitrage doctrinal impose « fala connosco » collectif) ; (11) **Liens internes** vers 4 pages sœurs (avaria, certificação, quadro, disjuntor) + calculadora + zonas + index ; (12) **Suppression des violations R11/R12/R145/FPP** : « Resposta Mediante Confirmação Por Telefone » (3 occurrences) purgé · « Grohe/Roca/Sanindusa » (marques CANAL interdite sur site élec) purgé · « qualquer problema de canalização ou eletricidade » (FPP scope-electric-on-plumbing) purgé · « atendimento 24h/7 mediante confirmação » (R145 INTERDIT) purgé · « técnicos com experiência » (formulation vague → « a nossa equipa ») remplacé · fourchette « 15-35€ deslocação » (incohérence) purgée → tabela Z1-Z6 canonique · « orçamento é sempre gratuito » (R12 PRICING §60) → « orçamento por escrito antes da intervenção, sem surpresas ». NAP +35=105€ + 70€/h + DGEG + équipement EXACT élec + Z3 source-of-truth préservés. Mesure impact J+7 via gsc-trajectoire-cron.sh (recette : pos < 4 = win, pos > 10 = rollback possible). R7 : PR DRAFT + STOP merge + GO explicite Filipe. **⚠️ HOTSPOT** : le fichier `blog/tomada-nao-funciona.html` était simultanément en cours d'édition par un agent concurrent (kanban sibling t_911bf00b "tomada parou de funcionar") — j'ai préservé son WIP (2 FAQ + title/description/JSON-LD) à `/tmp/kanban-sibling-recovery/tomada-nao-funciona.html.sibling-wip` (551 lignes) après que mon checkout a glissé sur sa branche. Restauré `blog/tomada-nao-funciona.html` à HEAD puis commit isolé sur ma branche. Aucun de ses changements n'a été committé par moi — à restaurer quand l'agent sibling reprend sa branche. | Témoins AVANT/APRÈS sur `public/eletricista-urgente-braganca.html` (26 403 octets / 2 061 lignes vs 15 620 octets / 101 lignes) : grep `mediante` 3 → **0** · grep `Grohe\|Roca\|Sanindusa` 1 → **0** · grep `canalização\|hidraul` 1 → **0** · grep `orçamento gratuito\|orçamento gratis\|gratuita e sem compromisso` 1 → **0** · grep `em [0-9]+ minutos\|tempo médio` 0 → 0 · présence `70€/h` 0 → **8** · présence `Zona 3 · 35€` 0 → **1** · présence `+50%` 0 → **1** · présence `orçamento por escrito antes` 0 → **6** · présence `DGEG TRIESP n.º 90062` 0 → **4** · présence `Fluke T6-1000` 0 → **1** · présence `Megger MFT1741+` 0 → **1** · présence `FLIR E96` 0 → **1** · présence `932 321 892` 0 → **4** · présence `Norte Reparos` 0 → **5** · `@type` JSON-LD : 1 (WebSite cassé) → 13 (WebSite + Organization + LocalBusiness + Service + FAQPage + 5 Question + 5 Answer + 2 ContactPoint + 2 City + 2 PostalAddress + 1 ImageObject + 1 ServiceChannel + 1 OpeningHoursSpecification) · `@context` JSON-LD mal formé → `https://schema.org` correctement cité 1 fois. **⚠️ DISCLAIMER FAT/META** : nouvelle page fait 26 KB (cible SEO référence ~15-30 KB), aucune invention client/chantier (R11), aucun délai chiffré (R145), géo-neutre (pas d'adresse précise, géoloc dynamique), NAP unique 932, équipement EXACT élec. Suit gabarit §13 AGENTS.md + Doctrine Transparence Radicale §12. **Validation production requise** : Filipe (STOP R3 avant batch) avant merge sur `main`. **Branche** : `fix/eu-braganca-scope-electric-fpp-t_938087ad` (réutilisée — kanban n'a pas créé de branche dédiée, le WIP propre est isolé à `public/eletricista-urgente-braganca.html`). **Mesure d'impact** : `gsc-trajectoire-cron.sh` J+7 (recette : pos < 4 = win). | ⏳ PR DRAFT, await STOP Filipe |
...
**Dernière MAJ** : 2026-09-10 (t_c881984b)
