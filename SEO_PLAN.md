...
|---|---|---|---|---|---|---|
| 2026-07-13 | Hermes | **Design EU statique — pilote premium.css hazard (indigo+rouge+jaune), divergé CU** | Création `premium.css` (497 lignes) avec palette hazard divergée CU (base indigo `#1A1A3A` filiation ENR + accent rouge `#FF2D1F` CTA/bandeau + jaune volt `#FACC15` éclair glow jamais sur body). Migration homepage `index.html` : retrait `<style>` inline (-347 lignes), ajout `<link rel="stylesheet" href="/premium.css">`, ajout `bandeau-hazard` (signature EU) + `sticky-call` mobile-first. Création `design-preview-eu.html` (noindex, showroom validation Filipe). Commit atomique `091378272` sur branche `feat/design-eu-static`, PR DRAFT #137 ouverte, **STOP push**. GATES 23/23 PASS : md5 EU `ed963bf3` ≠ CU `b37de210`, tel:+351****1892 ×5 préservé, schema Electrician préservé, theme-color #d97706→#1A1A3A, 0 hazard sur body text (contraste AA), 0 délai chiffré R145, 0 rating/avis R11. | Leçon #353 (design différenciation non-cassant) : EU n'avait PAS de premium.css dédié (contrairement à CU) — le CSS vivait inline dans chaque `<style>` des 1946 HTML. **NE PAS batcher** les 1946 fichiers existants (R12 §14 refonte en attente — décision business Philippe) — pilote-first sur 1 page + 1 showroom = réversible. Pattern de migration documenté en tête de `premium.css` : adopter via `<link rel="stylesheet" href="/premium.css">` + suppression `<style>` legacy. | Branche `feat/design-eu-static` pushée origin, PR #137 DRAFT. Leçon #353 à coder (cf memory + skill `norte-pr-design-quality`). | ⏳ PR #137 DRAFT — attente validation visuelle Filipe avant merge |
